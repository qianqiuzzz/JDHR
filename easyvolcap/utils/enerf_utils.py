...
class ConvBnReLU(nn.Module):
    def __init__(self, in_channels, out_channels,
        ...
        self.relu = nn.ReLU()  # might pose problem for pass through optimization, albeit faster
...
class FeatureNet(nn.Module):
    def __init__(self, norm_actvn=nn.BatchNorm2d, test_using_train: bool = True):
        ...
        #self.smooth1 = nn.Conv2d(32, 16, 3, padding=1)# mark: no used, del
    ...
    def _upsample_add(self, x, y):
        return nn.interpolate(x, scale_factor=2, mode='bilinear', align_corners=False) + y
    ...
    def execute(self, x: jt.Var):
        ...
        #feat1 = self.smooth1(feat1)#mark: no used, del
...
class MinCostRegNet(nn.Module):
    ...
    def execute(self, x, use_vox_feat=True):
        conv0 = self.conv0(x)
        conv2 = self.conv2(self.conv1(conv0))
        conv4 = self.conv4(self.conv3(conv2))
        x = conv4
...
class NeRF(nn.Module):
    ...
    def execute(self, vox_feat, img_feat_rgb_dir):
        B, N_points, N_views = img_feat_rgb_dir.shape[:-1]
        img_feat = self.agg(img_feat_rgb_dir)
        S = img_feat_rgb_dir.shape[2]

        vox_img_feat = jt.cat((vox_feat, img_feat), dim=-1)

        x = self.lr0(vox_img_feat)
        for i in range(len(self.lrs)):
            x = self.lrs[i](x)
        sigma = self.sigma(x)
        x = jt.cat((x, vox_img_feat), dim=-1)
        x = x.view(B, -1, 1, x.shape[-1]).repeat(1, 1, S, 1)
        x = jt.cat((x, img_feat_rgb_dir), dim=-1)
        color_weight = nn.softmax(self.color(x), dim=-2)
        color = jt.sum((img_feat_rgb_dir[..., -7:-4] * color_weight), dim=-2)
        return jt.cat([color, sigma], dim=-1)
...
class FeatureAgg(nn.Module):
    ...
    def execute(self, img_feat_rgb_dir: jt.Var):
        ...
        var_feat = jt.var(img_feat_rgb, dim=-2).view(B, -1, 1, self.feat_ch).expand(-1, -1, S, -1)
        avg_feat = jt.mean(img_feat_rgb, dim=-2).view(B, -1, 1, self.feat_ch).expand(-1, -1, S, -1)

        feat = jt.cat([img_feat_rgb, var_feat, avg_feat], dim=-1)
        global_feat = self.global_fc(feat)
        agg_w = nn.softmax(self.agg_w_fc(global_feat), dim=-2)
...
class conv(nn.Module):
    ...
    def execute(self, x):
        return nn.elu(self.bn(self.conv(x)), inplace=True)
...
class upconv(nn.Module):
    ...
    def execute(self, x):
        x = nn.interpolate(x, scale_factor=self.scale, align_corners=True, mode='bilinear')
        return self.conv(x)
...
class ResUNet(nn.Module):
    ...
    def skipconnect(self, x1, x2):
        ...
        x1 = nn.pad(x1, (diffX // 2, diffX - diffX // 2,
