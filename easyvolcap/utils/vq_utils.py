...
def l2norm(t):
    return misc.normalize(t, p=2, dim=-1)
...
def ema_inplace(moving_avg, new, decay):
    moving_avg.data.multiply_(decay).add_(new*(1 - decay))
...
def sample_vectors(samples, num):
    ...
    if num_samples >= num:
        indices = jt.randperm(num_samples)[:num]
    else:
        indices = jt.randint(0, num_samples, (num,))

    return samples[indices]


def batched_sample_vectors(samples, num):
    return jt.stack([sample_vectors(sample, num) for sample in samples.unbind(dim=0)], dim=0)
...
def sample_multinomial(total_count, probs):#£¿£¿£¿£¿
    ...
    sample = jt.empty(probs.size(), dtype='int32')

    for i, p in enumerate(probs):
        s = jt.binomial(total_count, p / remainder)
        sample[i] = s
        total_count -= s
        remainder -= p
...
def all_gather_sizes(x, dim):
    size = jt.int32(x.shape[dim], dtype=jt.long, device=x.device)
    all_sizes = [jt.empty_like(size) for _ in range(distributed.get_world_size())]
    distributed.all_gather(all_sizes, size)

    return jt.stack(all_sizes)
...
def sample_vectors_distributed(local_samples, num):
    ...
    if rank == 0:
        ...
        samples_per_rank = jt.empty_like(all_num_samples)
    ...
    return jt.cat(all_samples, dim=0)


def batched_bincount(x, *, minlength):
    batch, dtype, device = x.shape[0], x.dtype, x.device
    target = jt.zeros(batch, minlength, dtype=dtype, device=device)
    values = jt.ones_like(x)
...
def orthogonal_loss_fn(t):
    ...
    identity = repeat(jt.eye(n, device=t.device), 'i j -> h i j', h=h)
...
class EuclideanCodebook(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.decay = decay
        init_fn = uniform_init if not kmeans_init else jt.zeros
        ...
        self.register_buffer('initted', jt.Var([not kmeans_init]))
        self.register_buffer('cluster_size', jt.zeros(num_codebooks, codebook_size))
    ...
    def init_embed_(self, data):
        ...
        self.initted.data.copy_(jt.Var([True]))

    def replace(self, batch_samples, batch_mask):
        batch_samples = l2norm(batch_samples)

        for ind, (samples, mask) in enumerate(zip(batch_samples.unbind(dim=0), batch_mask.unbind(dim=0))):
            if not misc.any(mask):
                continue
...
class VectorQuantize(nn.Module):
    ...
    def forward(self, x, weight=None, verbose=False):
        ...
        loss = jt.Var([0.], device=device, requires_grad=self.training)
        ...
        if self.training:
            ...
            if self.orthogonal_reg_weight > 0:
                ...
                if self.orthogonal_reg_active_codes_only:
                    ...
                    unique_code_ids = jt.unique(embed_ind)
                    codebook = codebook[unique_code_ids]

                num_codes = codebook.shape[0]
                if exists(self.orthogonal_reg_max_codes) and num_codes > self.orthogonal_reg_max_codes:
                    rand_ids = jt.randperm(num_codes, device=device)[:self.orthogonal_reg_max_codes]
...
class CosineSimCodebook(nn.Module):
    def __init__(
        ...
        if not kmeans_init:
            ...
            embed = jt.zeros(num_codebooks, codebook_size, dim)
...
class VectorQuantizationEmbedding(nn.Module):
    ...
    def forward(self, z_e_x: jt.Var) -> jt.Var:
        if self.K == 0: return z_e_x
        z_e_x_ = z_e_x.permute(0, 2, 3, 1).contiguous()
        latents = vector_quantization(z_e_x_, self.embedding.weight)
        return latents

    def straight_through(self, z_e_x: jt.Var) -> Tuple[jt.Var, jt.Var]:
        ...
        z_q_x_bar_flatten = jt.index_select(self.embedding.weight,
                                               dim=0, index=indices)
...
class VectorQuantization(Function):
    @ staticmethod
    def forward(ctx, inputs: jt.Var, codebook: jt.Var):
        with jt.no_grad():
            embedding_size = codebook.size(1)
            inputs_size = inputs.size()
            inputs_flatten = inputs.view(-1, embedding_size)

            codebook_sqr = jt.sum(codebook ** 2, dim=1)
            inputs_sqr = jt.sum(inputs_flatten ** 2, dim=1, keepdim=True)

            # Compute the distances to the codebook
            distances = jt.addmm(codebook_sqr + inputs_sqr,
                                    inputs_flatten, codebook.t(), alpha=-2.0, beta=1.0)

            _, indices_flatten = jt.min(distances, dim=1)
...
class VectorQuantizationStraightThrough(Function):
    ...
    def forward(ctx, inputs, codebook):
        ...
        codes_flatten = misc.index_select(codebook, dim=0,
    ...
    def backward(ctx, grad_output, grad_indices):
        ...
        if ctx.needs_input_grad[1]:
            ...
            grad_codebook = jt.zeros_like(codebook)
