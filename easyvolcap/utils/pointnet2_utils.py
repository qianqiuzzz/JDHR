...
def index_points(points, idx):
    ...
    repeat_shape[0] = 1
    batch_indices = jt.arange(B, dtype='int32').view(view_shape).repeat(repeat_shape)
...
def farthest_point_sample(xyz, npoint):
    ...
    distance = jt.ones(B, N).to(device) * 1e10
...
def query_ball_point(radius, nsample, xyz, new_xyz):
    ...
    B, N, C = xyz.shape
    _, S, _ = new_xyz.shape
    group_idx = jt.arange(N, dtype='int32').view(1, 1, N).repeat([B, S, 1])
...
def sample_and_group(npoint, radius, nsample, xyz, points, returnfps=False):
    ...
    if points is not None:
        ...
        new_points = jt.cat([grouped_xyz_norm, grouped_points], dim=-1)  # [B, npoint, nsample, C+D]
    else:
...
def sample_and_group_all(xyz, points):
    ...
    """
    B, N, C = xyz.shape
    new_xyz = jt.zeros(B, 1, C)
    grouped_xyz = xyz.view(B, 1, N, C)
    if points is not None:
        new_points = jt.cat([grouped_xyz, points.view(B, 1, N, -1)], dim=-1)
    else:
...
class PointNetSetAbstraction(nn.Module):
    ...
    def execute(self, xyz, points):
        ...
        for i, conv in enumerate(self.mlp_convs):
            bn = self.mlp_bns[i]
            new_points = nn.relu(bn(conv(new_points)))

        new_points = jt.max(new_points, 2)
    ...
    def execute(self, xyz, points):
        ...
        for i, radius in enumerate(self.radius_list):
            ...
            if points is not None:
                ...
                grouped_points = jt.cat([grouped_points, grouped_xyz], dim=-1)
            else:
            ...
            for j in range(len(self.conv_blocks[i])):
                ...
                grouped_points = nn.relu(bn(conv(grouped_points)))
            new_points = jt.max(grouped_points, 2)  # [B, D', S]
            new_points_list.append(new_points)

        new_xyz = new_xyz.permute(0, 2, 1)
        new_points_concat = jt.cat(new_points_list, dim=1)
...
class PointNetFeaturePropagation(nn.Module):
    ...
    def execute(self, xyz1, xyz2, points1, points2):
        ...
        if points1 is not None:
            ...
            new_points = jt.cat([points1, interpolated_points], dim=-1)
        else:
...
class PointNeRFAggregator(nn.Module):
    ...
    def execute(self, xyz: jt.Var, pcd: jt.Var, feat: jt.Var):
        close = misc.knn(xyz, pcd,self.K)  # B, S, K
        ...
        feat = jt.cat([feat, xyz[..., None, :] - pts], dim=-1)
...
class PointNet2Aggregator(nn.Module):
    ...
    def execute(self, xyz: jt.Var, feat: jt.Var):
        # B, C, N = xyz.shape
        ...
        x = self.drop1(nn.relu(x))
        x = self.fc2(x)
        x = self.bn2.eval()(x)  # !: Maybe only one batch during training
        x = self.drop2(n.relu(x))
        x = self.fc3(x)
        x = nn.log_softmax(x, -1)
