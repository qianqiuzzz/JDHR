...
def get_mesh(verts: jt.Var, faces: jt.Var, uv: jt.Var = None, img: jt.Var = None, colors: jt.Var = None, normals: jt.Var = None, filename: str = "default.ply"):
    ...
    if colors is None:
        # colors = verts
        colors = face_normals(jt.array(verts), jt.array(faces).long()) * 0.5 + 0.5
...
def get_tensor_mesh_data(verts: jt.Var, faces: jt.Var, uv: jt.Var = None, img: jt.Var = None, uvfaces: jt.Var = None):
...
def load_mesh(filename: str, device='cuda', load_uv=False, load_aux=False, backend='pytorch3d'):
    ...
    #    v = jt.array(mesh['verts'])
    #    f = jt.array(mesh['faces'])

    #    if load_uv:
    #        vm = jt.array(mesh['uvs'])
    #        fm = jt.array(mesh['uvfaces'])
    #else:
...
def export_pts(pts: jt.Var, color: jt.Var = None, normal: jt.Var = None, scalars: dotdict = dotdict(), filename: str = "default.ply", skip_color: bool = False, **kwargs):
...
def export_lines(verts: jt.Var, lines: jt.Var, color: jt.Var = None, filename: str = 'default.ply'):
    if color is None:
        color = verts
...
def export_camera(c2w: jt.Var, ixt: jt.Var = None, col: jt.Var = jt.Var([50, 50, 200]), axis_size=0.10, filename: str = 'default.ply'):
    verts = []
    lines = []
    rgbs = []

    def add_line(p0: jt.Var, p1: jt.Var, col: jt.Var):
        ...
        col = jt.broadcast(col, sh + (3,))
        rgbs.append(col)
        rgbs.append(col)

        # Add the faces
        #b=jt.array(np.array(p0.shape))
        new = p0.numpy().size // 3  # number of new elements
        curr = new * (len(verts) - 2)  # assume all previous elements are of the same size
        start = jt.arange(curr, curr + new)
        end = jt.arange(curr + new, curr + new * 2)
        line = jt.stack([start, end], dim=-1)  # NM, 2
        line = line.view(sh + (2,))
        lines.append(line)
    ...
    add_line(p, p + axis_size * c2w[..., 0], jt.Var([255, 64, 64]))
    add_line(p, p + axis_size * c2w[..., 1], jt.Var([64, 255, 64]))
    add_line(p, p + axis_size * c2w[..., 2], jt.Var([64, 64, 255]))
    add_line(p, a, col)
    add_line(p, b, col)
    add_line(p, c, col)
    add_line(p, d, col)
    ...
    verts = jt.stack(verts)
    lines = jt.stack(lines)
    rgbs = jt.stack(rgbs)

    export_lines(verts, lines, rgbs, filename=filename)


def export_mesh(verts: jt.Var, faces: jt.Var, uv: jt.Var = None, img: jt.Var = None, uvfaces: jt.Var = None, colors: jt.Var = None, normals: jt.Var = None, filename: str = "default.ply", subdivision=0):
...
def export_o3d_pts(pts: jt.Var, filename: str = "default.ply"):
...
def export_o3d_pcd(pts: jt.Var, rgb: jt.Var, normal: jt.Var, filename="default.ply"):
    import open3d as o3d
...
def export_pcd(pts: jt.Var, rgb: jt.Var, occ: jt.Var, filename="default.ply"):
    import pandas as pd
    from pyntcloud import PyntCloud
...
def save_img(index, dir, img: jt.Var, remove_batch=True, remap=False, flip=False):
...
def to_cuda(batch, device="cuda", ignore_list: bool = False) -> jt.Var:
...
#        if cond(x):
#            batch = jt.array(batch).to(x, non_blocking=True)
#    else:
...
def to_x(batch, x: str) -> Union[jt.Var, dotdict[str, jt.Var]]:#?????
    if isinstance(batch, (tuple, list)):
        batch = [to_x(b, x) for b in batch]
    elif isinstance(batch, dict):
        batch = dotdict({k: to_x(v, x) for k, v in batch.items()})
...
def to_tensor(batch, ignore_list: bool = False) -> Union[jt.Var, dotdict[str, jt.Var]]:
...
def to_cpu(batch, non_blocking=False, ignore_list: bool = False) -> jt.Var:
    if isinstance(batch, (tuple, list)) and not ignore_list:
...
def remove_batch(batch) -> Union[jt.Var, np.ndarray]:
    if isinstance(batch, (tuple, list)):
        batch = [remove_batch(b) for b in batch]
    elif isinstance(batch, dict):
        batch = dotdict({k: remove_batch(v) for k, v in batch.items()})
    elif isinstance(batch, (jt.Var, np.ndarray)):  # numpy and others
        batch = batch[0]
    else:
        batch = jt.array(batch)[0]
    return batch


def add_batch(batch) -> Union[jt.Var, np.ndarray]:
    if isinstance(batch, (tuple, list)):
        ...
        batch = jt.array(batch)[None]
    return batch


def add_iter(batch, iter, total) -> Union[jt.Var, np.ndarray]:
...
def add_scalar(batch, value, name) -> Union[jt.Var, np.ndarray]:
    if isinstance(batch, (tuple, list)):
        for b in batch:
            add_scalar(b, value, name)

    if isinstance(batch, dict):
        batch[name] = jt.Var(value)
        batch['meta'][name] = np.array(value)
    return batch
...
def get_rigid_transform(pose: np.ndarray, joints: np.ndarray, parents: np.ndarray):
    ...
    pose, joints, parents = jt.Var([pose, joints, parents])
...
def save_image(img_path: str, img: np.ndarray, jpeg_quality=75, png_compression=9, save_dtype=np.uint8):
    if isinstance(img, jt.Var): img = img.detach().numpy()  # convert to numpy arrays
...
def list_to_tensor(x: list, device='cuda'): return jt.array(list_to_numpy(x)).to(device, non_blocking=True)  # convert list of numpy arrays of HWC to BCHW


def tensor_to_list(x: jt.Var): return numpy_to_list(x.detach().cpu().numpy())  # convert tensor of BCHW to list of numpy arrays of HWC
...
def batch_project(xyz, K, RT, return_depth=False):
    ...
    xyz_hom = jt.cat([xyz, jt.ones(B, N, 1)], dim=-1)  # [B, N, 4]

    # Projection matrix
    P = jt.matmul(K, RT)  # [B, 3, 4]

    # Project points
    xyz_proj = jt.linalg.einsum('bni,bij->bnj', P, xyz_hom.permute(0, 2, 1)).permute(0, 2, 1)  # [B, N, 3]
...
def affine_inverse(m: np.ndarray):
    ...
    return affine_inverse(jt.array(m)).numpy()
...
def load_image_from_bytes(buffer: np.ndarray, ratio=1.0, normalize=False, decode_flag=cv2.IMREAD_UNCHANGED):
    ...
    def normalize_image(image):
        #image = jt.array(image)  # pytorch is significantly faster than np
        if image.ndim >= 3 and image.shape[-1] >= 3:
            image[..., :3] = image[..., [2, 1, 0]]
        image = image / misc.iinfo(image.dtype).max
        image = image#.float()
        return image#.numpy()
    ...
    if isinstance(buffer, jt.Var):
        buffer = buffer.numpy()
    #buffer=buffer
    #print("buffer",len(buffer),buffer.shape)
    #print(buffer)
    if(len(buffer)<2):
        buffer = buffer[0].astype(np.uint8)
    else:
...
#class UnstructuredTensors(jt.Var):
    # https://github.com/pytorch/pytorch/issues/13246#issuecomment-617140519
    # https://github.com/pytorch/pytorch/issues/69893
#    @staticmethod
#    def __new__(cls, bytes: Union[List[np.ndarray], List[jt.Var], np.ndarray], **kwargs):
#        """
#        Creates a new UnstructuredTensors object from the given bytes.

#        Args:
#        - bytes (Union[List[np.ndarray], List[jt.Var], np.ndarray]): The bytes to create the object from.
...
#        self = jt.Var.__new__(cls).to(dtype)

        # Remember accessing related configs
#        self.set_(jt.cat(bytes))  # flatten # sum(N)
#        self.lengths = jt.array([len(b) for b in bytes], dtype=jt.int32)  # N,
#        self.cumsums = jt.cat([jt.array([0]), jt.cumsum(self.lengths, dim=0)[:-1]])
#        return self
...
#        Returns:
#            jt.Var: A slice of the UnstructuredTensors object corresponding to the given index.
...
#            return jt.Var.__getitem__(self, slice(self.cumsums[index], self.cumsums[index] + self.lengths[index]))
#        else:
