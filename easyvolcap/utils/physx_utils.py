...
class Garment(dotdict):
    ...
    def __init__(self, v: jt.Var, f: jt.Var, vm: jt.Var, fm: jt.Var, material: StVKMaterial = StVKMaterial()):
...
def get_shape_matrix(tris: jt.Var):
    return jt.stack([tris[..., 0, :] - tris[..., 2, :],
                        tris[..., 1, :] - tris[..., 2, :],
                        ], dim=-1)


def deformation_gradient(tris: jt.Var, Dm_invs: jt.Var):
    Ds = get_shape_matrix(tris)
    return Ds @ Dm_invs


def get_matching_identity_matrix(F: jt.Var):
    # match shape of F
    shape = F.shape[:-2]
    I = init.eye(F.shape[-1], dtype=F.dtype)
    for i in shape:
        I = I[None]
...
def green_strain_tensor(F: jt.Var) -> jt.Var:
    I = get_matching_identity_matrix(F)
    return 0.5 * (F.transpose(-2,-1) @ F - I)
,-

def stretch_energy_constraints(v: jt.Var, garment: Garment, **kwargs):
    '''
    v: B, V, 3
...
def bending_energy_constraints(v: jt.Var, garment: Garment, **kwargs):
    # XPBD step
...
def xpbd_constraints(func: Callable[..., jt.Var], triangles: jt.Var, inv_mass: jt.Var, garment: Garment, accum_lambda: jt.Var = None, delta_t: float = 1, compliance: float = 0):
    ...
    if accum_lambda is None:
        accum_lambda = jt.zeros_like(inv_mass)
...
def stretch_energy_components(triangles: jt.Var, garment: Garment):
    '''
    triangles: B, F, 3, 3
...
def bending_energy_components(v: jt.Var, connected_triangles: jt.Var, garment: Garment) -> jt.Var:
    '''
    ...
    n = jt.cross(connected_triangles[:, :, :, 1, :] - connected_triangles[:, :, :, 0, :], connected_triangles[:, :, :, 2, :] - connected_triangles[:, :, :, 1, :])
    n0 = n[:, :, 0]
    n1 = n[:, :, 1]
...
def stretch_energy(v: jt.Var, garment: Garment):
    '''
    v: B, V, 3
    ...
    return jt.sum(energy) / B


def bending_energy(v: jt.Var, garment: Garment):
    '''
    v: B, V, 3
    ...
    # Compute dihedral angle between faces
    cos = (n0 * n1).sum(dim=-1)  # dot product, B, E
    sin = (e_norm * jt.cross(n0, n1)).sum(dim=-1)  # B, E
    theta = jt.arctan2(sin, cos)  # B, E
...
def gravity_energy(x: jt.Var, mass: jt.Var, g=9.81):
    # S, V, 3
    # U = m * g * h
    U = g * mass * (x[:, :, 1] + 0)  # mgh

    return jt.sum(U) / x.shape[0]


def inertia_term(x_next: jt.Var,
                 x_curr: jt.Var,
                 v_curr: jt.Var,
                 mass: jt.Var,
                 delta_t
...
def dynamic_term(x_next: jt.Var,
                 x_curr: jt.Var,
                 v_curr: jt.Var,
                 mass: jt.Var,
                 delta_t,
    ...
    gravitational_acceleration = jt.Var(gravitational_acceleration, dtype=x_next.dtype)
...
def inertia_term_sequence(x: jt.Var,
                          mass: jt.Var,
                          delta_t: float,
                          method: Callable[[jt.Var, jt.Var, jt.Var, jt.Var, float], jt.Var] = inertia_term,
                          compliance: float = 1.0,
                          ):
    """
    x: jt.Var of shape [batch_size, num_frames, num_vertices, 3]
    """
    B = x.shape[0]
    V = x.shape[-2]
    # Compute velocities
    ...
    zeros = jt.zeros([B, 1, V, 3], dtype=x.dtype)
    v_curr = jt.cat([zeros, v_next[:, :-1]], dim=1)  # B, T-1, V, 3
