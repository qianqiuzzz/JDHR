...
def unmerge_faces(faces: jt.Var, *args):
    ...
    stack = jt.Var(np.column_stack(stackable))
    ...
    remap = jt.zeros(len(order), dtype=jt.int32)
    remap[order] = jt.arange(len(order))
...
def merge_faces(faces, *args, n_verts=None):
    ...
    result.append(jt.arange(n_verts))
    ...
    for arg in args:
        ...
        masks = jt.zeros((3, n_verts), dtype=jt.int32)
        # set the mask using the unmodified face indexes
        for i, f, a in zip(range(3), faces.permute(*jt.arange(faces.ndim - 1, -1, -1)), arg.permute(*jt.arange(arg.ndim - 1, -1, -1))):
            masks[i][f] = a
        ...
        result.append(jt.Var(np.median(masks, dim=0)[0]))

    return result


def meshes_attri_laplacian_smoothing(meshes: Meshes, attri: jt.Var, method: str = "uniform"):
    if meshes.isempty():
        return jt.float32(
    ...
    with jt.no_grad():
        if method == "uniform":
            ...
            if method == "cot":
                norm_w = jt.sum(L, dim=1).to_dense().view(-1, 1)
                idx = norm_w > 0
                norm_w[idx] = 1.0 / norm_w[idx]
            else:
                L_sum = jt.sum(L, dim=1).to_dense().view(-1, 1)
...
class RegisterSDFGradient(Function):
    @staticmethod
    def forward(ctx: FunctionCtx, verts: jt.Var, decoder: Callable[[jt.Var], jt.Var], chunk_size=65536):
    ...
    @staticmethod
    def backward(ctx: FunctionCtx, grad: jt.Var):
        chunk_size = ctx.chunk_size
        decoder: Callable[[jt.Var], jt.Var] = ctx.decoder
        verts: jt.Var = ctx.saved_tensors[0]

        verts = verts.detach().requires_grad_()  # should not affect the original verts
        with jt.enable_grad():
            sdf = jt.cat([decoder(verts[i:i + chunk_size]) for i in range(0, verts.shape[0], chunk_size)])
            norm = normalize(take_gradient(sdf, verts, create_graph=False, retain_graph=True))  # N, 3
            grad = -jt.linalg.einsum('ni,ni->n', norm, grad) * sdf.view(verts.shape[0])  # N
            loss = grad.sum()
...
def differentiable_marching_cubes(points: jt.Var, decoder: Callable[[jt.Var], jt.Var], chunk_size=65536):
    """
    ...
    upper = points.max(dim=0, keepdim=True)
    lower = points.min(dim=0, keepdim=True)
    ...
    verts = jt.array(verts.astype(np.float32)).to( non_blocking=True)
    faces = jt.array(faces.astype(np.int32)).to( non_blocking=True)
    verts = verts * (upper - lower) / (jt.Var(sh[:-1]) - 1) + lower

    verts = register_sdf_gradient(verts, decoder, chunk_size)  # literally a state switch, no other stuff
    return verts, faces


def face_normals(v: jt.Var, f: jt.Var):
    # compute faces normals w.r.t the vertices (considering batch dimension)
    tris = multi_gather_tris(v, f)

    # Compute face normals
    v0, v1, v2 = jt.split(tris, split_size_or_sections=1, dim=-2)
    v0, v1, v2 = v0[..., 0, :], v1[..., 0, :], v2[..., 0, :]
    e1 = v1 - v0
    e2 = v2 - v1
    face_normals = jt.cross(e1, e2)
...
def get_face_connectivity(faces: jt.Var):
    '''
    ...
    vert = he.vert
    edge = he.edge
    hedg = jt.arange(he.HE)  # HE, :record half edge indices
    face = hedg // 3
    ...
    args = edge_manifold.argsort()[0]  # 00, 11, 22 ...
...
def get_edge_length(v, e):
    ...
    return jt.norm(v0 - v1, dim=-1)


def get_vertex_mass(v: jt.Var, f: jt.Var, density: float, areas=None):
    '''
    ...
    vertex_masses = jt.zeros(v.shape[:-1], dtype=v.dtype)
...
def get_face_areas(v: jt.Var, f: jt.Var) -> jt.Var:
    v0 = v[f[..., 0]]
    v1 = v[f[..., 1]]
    v2 = v[f[..., 2]]
    ...
    return jt.norm(jt.cross(u, v), dim=-1) / 2.0


def loop_subdivision(v: jt.Var, f: jt.Var, steps=2):
...
def segment_mesh(verts: jt.Var,
    ...
    e, i, c = get_edges(f)
    if smoothing == 'edge':
        svi = e[c != 2].reshape(-1,).unique()  # selected vertices' indices
    else:
        svi = None
    ...
    # Convert back to jt.Var
    v, f = jt.float(v), jt.int32(f)

    return v, f


def icp_loss(v: jt.Var,  # input points
             t: jt.Var,  # triangles
             n0: jt.Var,  # normal of the input points: v
             n1: jt.Var,  # normal of the target triangles: t
...
def bidirectional_icp_fitting(v0: jt.Var,
                              f0: jt.Var,
                              v1: jt.Var,
                              f1: jt.Var,
    ...
    if boundary_focus:
        ...
        svi0 = e0[c0 != 2].reshape(-1,).unique()  # selected vertices' indices: boundary
        svi1 = e1[c1 != 2].reshape(-1,).unique()  # selected vertices' indices: boundary

        # dialte the boundary edge selection
        vm0 = jt.zeros(v0.shape[:-1], dtype=jt.bool)
        vm1 = jt.zeros(v1.shape[:-1], dtype=jt.bool)
        vm0[svi0] = True
        vm1[svi1] = True
        A0 = adjacency(v0, e0)
        A1 = adjacency(v1, e1)
...
def halfedge_loop_subdivision(halfedge: dotdict[str, jt.Var], is_manifold=False):
    ...
    verts: jt.Var = halfedge.verts  # V, 3
    twin: jt.Var = halfedge.twin  # HE,
    vert: jt.Var = halfedge.vert  # HE,
    edge: jt.Var = halfedge.edge  # HE,
    ...
    # assign empty memory
    ntwin = jt.empty(NHE,  dtype=vert.dtype)
    nvert = jt.empty(NHE,  dtype=vert.dtype)
    nedge = jt.empty(NHE,  dtype=vert.dtype)
    ...
    nedge[i0] = 2 * edge + (hedg < twin).int32()
    nedge[i1] = 2 * E + hedg
    nedge[i2] = 2 * edge_prev + (prev > twin_prev).int32()
    nedge[i3] = nedge[i1]
    ...
    if not is_manifold:
        ...
        non_manifold_mask = manifold_mask.logical_not()
    ...
    beta = (1 / velance) * (5 / 8 - (3 / 8 + 1 / 4 * jt.cos(2 * math.pi / velance))**2)
    ...
    if not is_manifold:
        ...
        non_manifold_prev_twin_prev_mask = jt.zeros(HE,  dtype=manifold_mask.dtype)
...
def multiple_halfedge_loop_subdivision(halfedge: dotdict[str, jt.Var], steps=2, is_manifold=False):
    for i in range(steps):
        halfedge = halfedge_loop_subdivision(halfedge, is_manifold)
    return halfedge


def halfedge_to_triangle(halfedge: dotdict[str, jt.Var]) -> Tuple[jt.Var, jt.Var]:
    # assuming the mesh has clean topology? except for boundary edges
    # assume no boundary edge for now!
    ...
    e = jt.stack([vert, vert[next]], dim=-1)
    e01, e12, e20 = e[0::3], e[1::3], e[2::3]
    faces = jt.stack([e01[..., 0], e12[..., 0], e20[..., 0]], dim=-1)

    return verts, faces


def triangle_to_halfedge(verts: Union[jt.Var, None],
                         faces: jt.Var,
    ...
    e01 = jt.cat([v0, v1], dim=-1)  # (sum(F_n), 2)
    e12 = jt.cat([v1, v2], dim=-1)  # (sum(F_n), 2)
    e20 = jt.cat([v2, v0], dim=-1)  # (sum(F_n), 2)

    # stores the vertex indices for each half edge
    e = jt.empty(HE, 2, dtype=faces.dtype)
    e[0::3] = e01
    e[1::3] = e12
    e[2::3] = e20
    ...
    edges = jt.stack([jt.minimum(vert_next, vert), jt.maximum(vert_next, vert)], dim=-1)
    ...
    hedg = jt.arange(HE)  # HE, :record half edge indices

    if is_manifold:
        inds = edge.argsort()[0]  # 00, 11, 22 ...
        twin = jt.empty(inds.size(),dtype=inds.dtype)
        ...
        twin_manifold = jt.empty(inds.size(),dtype=inds.dtype)
        twin_manifold[args[0::2]] = inds[1::2]
        twin_manifold[args[1::2]] = inds[0::2]

        twin = jt.empty(HE, dtype=jt.int32)
        twin[manifold] = twin_manifold
        twin[manifold.logical_not()] = -counts[edge][manifold.logical_not()]  # non-manifold half edge mask, number of half edges stored in the twin
...
def winding_number(pts: jt.Var, verts: jt.Var, faces: jt.Var) -> jt.Var:
    """
    ...
    Parameters
    ----------
    pts    : jt.Var, (n_points, 3)
    verts  : jt.Var, (n_verts, 3)
    faces  : jt.Var, (n_faces, 3)
    ...
    sign = nn.sign((jt.cross(e0, e1) * u2).sum(dim=-1))
    ...
    l = jt.stack([l0, l1, l2], dim=-1)  # n_points, n_faces, 3
    ...
    winding = signed_solid.sum(dim=-1) / (4 * math.pi)  # n_points
...
def ray_stabbing(pts: jt.Var, verts: jt.Var, faces: jt.Var, multiplier: int = 1):
    """
    ...
    Parameters
    ----------
    ray_o : jt.Var(float), (n_rays, 3)
    verts : jt.Var(float), (n_verts, 3)
    faces : jt.Var(long), (n_faces, 3)
    """
    n_rays = pts.shape[0]
    pts = pts[None].expand(multiplier, n_rays, -1)
    pts = pts.reshape(-1, 3)
    ray_d = jt.rand_like(pts)  # (n_rays, 3)
...
def moller_trumbore(ray_o: jt.Var, ray_d: jt.Var, tris: jt.Var, eps=1e-8) -> Tuple[jt.Var, jt.Var, jt.Var]:
    """
    ...
    Parameters
    ----------
    ray_o : jt.Var, (n_batch, n_rays, 3)
    ray_d : jt.Var, (n_batch, n_rays, 3)
    tris  : jt.Var, (n_batch, n_faces, 3, 3)
    """
    E1 = tris[..., 1, :] - tris[..., 0, :]  # vector of edge 1 on triangle (n_faces, 3)
    E2 = tris[..., 2, :] - tris[..., 0, :]  # vector of edge 2 on triangle (n_faces, 3)

    # batch cross product
    N = jt.cross(E1, E2)  # normal to E1 and E2, automatically batched to (n_faces, 3)
    ...
    DA0 = jt.cross(A0, ray_d[..., :, None, :].expand(*A0.shape))  # (n_rays, n_faces, 3) x (n_rays, 3) -> (n_rays, n_faces, 3) no automatic broadcast
...
def winding_distance(pts: jt.Var, verts: jt.Var, faces: jt.Var, winding_th=0.45):
    from bvh_distance_queries import BVH

    winding = winding_number(pts, verts, faces)  # let's say its gonna be 0 or 1
    bvh = BVH()  # NOTE: wasteful!
    p: jt.Var = bvh(multi_gather_tris(verts[None], faces[None]), pts[None])[1][0, ...]  # remove last dimension
    d = (pts - p).norm(dim=-1)
    d = d
...
def bvh_distance(pts: jt.Var, verts: jt.Var, faces: jt.Var):
    from bvh_distance_queries import BVH
...
def winding_number_nooom(pts: jt.Var, verts: jt.Var, faces: jt.Var, quota_GB=15.0):
    ...
    winding = jt.cat(winding, dim=-1)

    return winding


def winding_distance_remesh(verts: jt.Var,
                            faces: jt.Var,

                            guide_verts: jt.Var = None,
                            guide_faces: jt.Var = None,
    ...
    close_verts = jt.minimum(d0, d1) < dist_th_verts
    pts = pts[close_verts]
    ...
    close_tris = jt.minimum(d0, d1) < dist_th_tris
    pts = pts[close_tris]
    d = d1[close_tris]
    ...
    # rgb = colormap(jt.Var(winding, device=verts.device) / 2 * 100 + 0.5)
    # export_pynt_pts(pts, rgb, filename='winding.ply')

    # undo two levels of filtering
    close_tris = close_tris
    cube_tris = jt.ones(close_tris.shape, dtype=jt.float) * -10
    cube_tris[close_tris] = winding_d
    close_verts = close_verts
    cube_verts = jt.ones(close_verts.shape, dtype=jt.float) * -10
    ...
    v = jt.Var(v, dtype=verts.dtype)
    f = jt.Var(f, dtype=faces.dtype)

    return v, f


def get_edges(faces: jt.Var):
    V = faces.max()
    F = faces.shape[0]
    HE = F * 3
    ...
    # stores the vertex indices for each half edge
    e = jt.empty(HE, 2, device=faces.device, dtype=faces.dtype)
    e[0::3] = e01
    e[1::3] = e12
    e[2::3] = e20
    vert = e[..., 0]  # HE, :record starting half edge
    ...
    e = jt.stack([u // V, u % V], dim=1)
    return e, i, c


def adjacency(verts: jt.Var, edges: jt.Var):
    V = verts.shape[0]

    e0, e1 = edges.unbind(1)

    idx01 = jt.stack([e0, e1], dim=1)  # (E, 2)
    idx10 = jt.stack([e1, e0], dim=1)  # (E, 2)
    idx = jt.cat([idx01, idx10], dim=0).tranpose()  # (2, 2*E)

    # First, we construct the adjacency matrix,
    # i.e. A[i, j] = 1 if (i,j) is an edge, or
    # A[e0, e1] = 1 &  A[e1, e0] = 1
    ones = jt.ones(idx.shape[1], dtype=jt.float32)
    # pyre-fixme[16]: Module `sparse` has no attribute `FloatTensor`.
    A = SparseVar(idx, ones, (V, V))
    return A


def laplacian(verts: jt.Var, edges: jt.Var):
    """
    ...
    idx01 = jt.stack([e0, e1], dim=1)  # (E, 2)
    idx10 = jt.stack([e1, e0], dim=1)  # (E, 2)
    idx = jt.cat([idx01, idx10], dim=0).transpose()  # (2, 2*E)
    ...
    deg = jt.sum(A, dim=1).to_dense()

    # We construct the Laplacian matrix by adding the non diagonal values
    # i.e. L[i, j] = 1 ./ deg(i) if (i, j) is an edge
    deg0 = deg[e0]
    # pyre-fixme[58]: `/` is not supported for operand types `float` and `Tensor`.
    deg0 = jt.ternary(deg0 > 0.0, 1.0 / deg0, deg0)
    deg1 = deg[e1]
    # pyre-fixme[58]: `/` is not supported for operand types `float` and `Tensor`.
    deg1 = jt.ternary(deg1 > 0.0, 1.0 / deg1, deg1)
    val = jt.cat([deg0, deg1])
    # pyre-fixme[16]: Module `sparse` has no attribute `FloatTensor`.
    L = SparseVar(idx, val, (V, V))

    # Then we add the diagonal values L[i, i] = -1.
    idx = jt.arange(V)
    idx = jt.stack([idx, idx], dim=0)
    ones = jt.ones(idx.shape[1], dtype=jt.float32)
    # pyre-fixme[16]: Module `sparse` has no attribute `FloatTensor`.
    L -= SparseVar(idx, ones, (V, V))

    return L


def laplacian_smoothing(v: jt.Var, e: jt.Var, inds: jt.Var = None, alpha=0.33, iter=90):
    for i in range(iter):
...
def isosurface_fitting(src_verts: jt.Var,  # assume no batch dim
                       src_faces: jt.Var,
                       tar_verts: jt.Var,
                       tar_faces: jt.Var,
                       f: Callable[[jt.Var, jt.Var, jt.Var], jt.Var] = winding_distance,  # might not be optimizable on well defined surface...
                       l: Callable[[jt.Var, jt.Var], jt.Var] = lambda x, y: (x - y).pow(2).sum(),
                       param_lambda: int = 29,
    ...
    for i in range(opt_iter):
        ...
        query = linear_gather(verts, jt.randperm(len(verts))[:chunk_size])
        level = f(
            query,
            tar_verts,
            tar_faces,
...
def average_edge_length(verts, faces, backend='pytorch'):
    ...
    if backend == 'pytorch':
        norm_func = partial(jt.norm, dim=-1)
    elif backend == 'np':
...
avg_pool_3d = jt.nn.AvgPool3d(2, stride=2)
upsample = jt.nn.Upsample(scale_factor=2, mode='nearest')
max_pool_3d = jt.nn.MaxPool3d(3, stride=1, padding=1)
...
def get_surface_sliding(
    ...
    if coarse_mask is not None:
        coarse_mask = coarse_mask.permute(2, 1, 0)[None, None].float()
    ...
    for i in range(N):
        for j in range(N):
            for k in range(N):
                ...
                points = jt.Var(np.vstack([xx.reshape(-1,), yy.reshape(-1,), zz.reshape(-1,)]).T, dtype=jt.float)

                def evaluate(points):
                    z = []
                    for _, pnts in enumerate(jt.split(points, 100000, dim=0)):
                ...
                if coarse_mask is not None:
                    # breakpoint()
                    points_tmp = points.permute(1, 2, 3, 0)[None]
                    current_mask = jt.nn.functional.grid_sample(coarse_mask, points_tmp)
                ...
                for pid, pts in enumerate(points_pyramid):
                    ...
                    if mask is None:
                        # only evaluate
                        if coarse_mask is not None:
                            pts_sdf = jt.ones_like(pts[:, 1])
                            valid_mask = (
                                jt.nn.functional.grid_sample(coarse_mask, pts[None, None, None])[0, 0, 0, 0] > 0
                            )
                            if valid_mask.any():
