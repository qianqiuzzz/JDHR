...
def compute_scale(Hc: int, Wc: int,
    ...
    Returns:
        ...
        real_scale: (jt.Var), (2, 1), new scaling factor with respect to the original image size
    """
    ...
    real_scale = jt.array([Wt / Wo, Ht / Ho])[..., None]  # (2, 1)

    if ret: return Ht, Wt, real_scale
    else: return Ht, Wt


def extremize_depth_map(near: jt.Var, far: jt.Var):
    """ Extremize the depth range to get the largest possible range
    Args:
        near: (jt.Var), (B, P, 1), near depth range for each pixel, may differ
        far: (jt.Var), (B, P, 1), far depth range for each pixel, may differ
    Returns:
        near_plane: (jt.Var), (B, 1, 1, 1), a uniform nearest depth plane for all pixels
        far_plane: (jt.Var), (B, 1, 1, 1), a uniform farthest depth plane for all pixels
    """
    # (batch_size, depth_hypotheses, height, width)
    near_plane = near.min(dim=-2)[..., None, None, :]  # (B, P, 1) -> (B, 1, 1, 1)
    far_plane = far.max(dim=-2)[..., None, None, :]  # (B, P, 1) -> (B, 1, 1, 1)
    return near_plane, far_plane


def upsample_depth_map(near: jt.Var, far: jt.Var,
                       Ht: int, Wt: int,
                       align_corners: bool = False):
    """ Upsample the previous depth range to guide depth estimation of current level
    Args:
        near: (jt.Var), (B, 1, Hc, Wc), low resolution near depth maps of previous level
        far: (jt.Var), (B, 1, Hc, Wc), low resolution far depth maps of previous level
        Ht, Wt: (int, int), the image height and width of current level
    Returns:
        near_plane: (jt.Var), (B, 1, Ht, Wt), upsampled nearest depth plane for current level
        far_plane: (jt.Var), (B, 1, Ht, Wt), upsampled farthest depth plane for current level
    """
...
def topk_selection(depth_hypo: jt.Var, depth_prob: jt.Var, k: int):
    """ Sample next level depth hypotheses between top k intervals
    Args:
        depth_hypo: (jt.Var), (B, D, Hc, Wc), depth hypotheses of current level
        depth_prob: (jt.Var), (B, D, Hc, Wc), probability of each depth hypothsis
        k: (int), number of depth hypotheses needs to sample for next level
    Returns:
        multi_hypo: (jt.Var), (B, 2*k, Ht, Wt), sampled depth hypotheses for next level
    """
    # Interval computation, for later sample interval subdivision
    hypo_interval = depth_hypo[:, 1:] - depth_hypo[:, :-1]  # (B, D-1, Hc, Wc)
    hypo_interval = jt.cat([hypo_interval, hypo_interval[:, -1:]], dim=1)  # (B, D, Hc, Wc)

    # Top k selection
    _, selected_inds = jt.topk(depth_prob, k=k, dim=1)  # (B, Hc, Wc)
    selected_hypo = jt.gather(depth_hypo, dim=1, index=selected_inds)  # (B, k, Hc, Wc)
    selected_intl = jt.gather(hypo_interval, dim=1, index=selected_inds)  # (B, k, Hc, Wc)
    ...
    multi_hypo = jt.cat([lower_hypo, upper_hypo], dim=1)  # (B, 2*k, Hc, Wc)

    return multi_hypo


def mixtured_laplacian_depth_regression(depth_repr: jt.Var, eps: float = 1e-2):
    ...
    Args:
        depth_repr: (jt.Var), (B, 5, H, W), bimodal depth distribution
        eps: (float), a small value to avoid numerical instability
    Returns:
        depth: (jt.Var), (B, H, W), estimated depth map
        std: (jt.Var), (B, H, W), estimated standard deviation, namely depth range
    """
    # Simplify notation
    ...
    sigma0 = jt.clamp(activation(depth_repr[:, 2, :]), eps, 1.0)  # (B, H, W)
    sigma1 = jt.clamp(activation(depth_repr[:, 3, :]), eps, 1.0)  # (B, H, W)
...
def unimodal_depth_regression(density: jt.Var, depth_hypo: jt.Var, is_debug: bool = False,
    ...
    Args:
        density: (jt.Var), (B, D, H, W), raw density of each pixel and each depth
        depth_hypo: (jt.Var), (B, D, H, W), depth planes or depth of samples
    Returns:
        depth: (jt.Var), (B, H, W), expectation of the unimodal distribution, namely the surface depth
        std: (jt.Var), (B, H, W), standard deviation of the unimodal distribution, possible range
    """
    # Use `softplus()` activation first, TODO: determine whether it is useful
    depth_prob = nn.softplus(density)  # (B, D, H, W)
...
def bimodal_depth_regression(density: jt.Var, z_vals: jt.Var, dist_default: float = 1.0,
    ...
    """ Perform depth regression from bimodal depth distribution, there is no explicit mixtured
        ...
        like NeRF, we perform the diffrentiable volume rendering execute and backward to constrain
        the output to be bimodal. (this version sounds more reasonable than the version above?)
    Args:
        density: (jt.Var), (B, D, H, W), volume density of each pixel in every depth plane
        z_vals: (jt.Var), (B, D, H, W), depth planes
        dist_default: (float), default distance between depth planes
    Returns:
        mode1: (jt.Var), (B, H, W), the estimated first mode of the bimodal depth distribution
        mode2: (jt.Var), (B, H, W), the estimated second mode of the bimodal depth distribution
    """
    ...
    if dist_default < 0:
        ...
        dist = jt.cat([dist, dist[..., -1:]], dim=-1)  # (B, P, D)
    else: dist = jt.full_like(density, dist_default)
    # Prepare for volume rendering: density * volume -> alpha (occupancy)
    occ = 1. - jt.exp(-nn.softplus(density) * dist)  # (B, P, D)

    # Perform execute and backward volume rendering to constrain the output to be bimodal
...
def bimodal_depth_curve_fn(whole: jt.Var, depth: jt.Var):
    p = 0.01  # 1% high and 1% lows
    n = int(whole.numel() * p)
    near = whole.reshape(-1,).topk(n, largest=False)[0].max()  # a simple version of percentile
    far = whole.reshape(-1,).topk(n, largest=True)[0].min()  # a simple version of percentile
...
def sample_feature_volume(s_vals: jt.Var, uv: jt.Var, feat_vol: jt.Var):  # scale are in cpu to avoid sync
    ...
    Args:
        s_vals: (jt.Var), (B, P, N), s_vals for sampled points
        uv: (jt.Var), (B, P, N, 2), processed 2d image coordinate of sampled points, make sure it has expanded (to fit with nerfacc)
        feat_vol: (jt.Var), (B, C, D, Ht, Wt), constructed image feature volume of target scale
    Returns:
        vox_feat: (jt.Var), (B, P, C), sampled feature from feature volume
    """
    # Deal with nasty shapes
    B, P, N = s_vals.shape
    ...
    uvd = jt.cat([uv, d], dim=-1)[:, None]  # (B, P, N, 3) -> (B, 1, P, N, 3)

    # Grid sample
    vox_feat = nn.grid_sample(feat_vol, uvd, padding_mode='border')[:, :, 0]  # (B, C, 1, P, N) -> (B, C, P, N)
    vox_feat = vox_feat.permute(0, 2, 3, 1).view(B, P * N, -1)  # (B, P, N, C) -> (B, P*N, C)
    return vox_feat


def sample_feature_image(xyz: jt.Var, src_feat: jt.Var, src_inps: jt.Var,
                         tar_ext: jt.Var, src_exts: jt.Var,
                         src_scale: Union[float, jt.Var], src_ixts: jt.Var,
    ...
    Args:
        xyz: (jt.Var), (B, P, 3), 3d world coordinates of sampled points
        src_feat: (jt.Var), (B, S, C, Hs, Ws), image feature of source views
        src_inps: (jt.Var), (B, S, 3, Hp, Wp), image of source views
        tar_ext: (jt.Var), (B, 4, 4), target camera extrinsic
        src_exts: (jt.Var), (B, S, 4, 4), source camera extrinsics
        src_scale: (Union[float, jt.Var]), (2, 1), scaling factor of source views
        src_ixts: (jt.Var), (B, S, 3, 3), source camera intrinsics
    Returns:
        img_feats: (jt.Var), (B, S, P, C), feature for all sampled points
    """
    # Deal with nasty shapes
    ...
    xyz1 = jt.cat([xyz, jt.ones_like(xyz[..., -1:])], dim=-1)  # (B, P, 4), homogeneous coordinates
    xyz1 = (xyz1[..., None, :, :] @ src_exts.transpose(-2,-1))  # (B, S, P, 4)
    xyzs = xyz1[..., :3] @ src_ixts.transpose(-2,-1)  # (B, S, P, 3) @ (B, S, 3, 3) -> (B, S, P, 3)
    xy = xyzs[..., :-1] / jt.clamp(xyzs[..., -1:], min=1e-6)  # (B, S, P, 2)
    x, y = xy.chunk(2, dim=-1)  # (B, S, P, 1), (B, S, P, 1)
    if correct_pix: xy = jt.cat([x / Wp * 2 - 1, y / Hp * 2 - 1], dim=-1)  # (B, S, P, 2)
    else: xy = jt.cat([x / (Wp - 1) * 2 - 1, y / (Hp - 1) * 2 - 1], dim=-1)  # (B, S, P, 2)

    # Actual sampling of the image features and rgb colors, (BS, C, 1, P) -> (B, S, C, P) -> (B, S, P, C)
    pts_inps = nn.grid_sample(src_inps.view(-1, 3, Hp, Wp), xy.view(-1, 1, P, 2), padding_mode='border', align_corners=not correct_pix).view(B, S, 3, P).permute(0, 1, 3, 2)
    pts_feat = nn.grid_sample(src_feat.view(-1, C, Hs, Ws), xy.view(-1, 1, P, 2), padding_mode='border', align_corners=not correct_pix).view(B, S, C, P).permute(0, 1, 3, 2)
    ...
    tar_cam = affine_inverse(tar_ext)[..., :3, -1:].transpose(-2,-1)  # (B, 1, 3)
    src_cams = affine_inverse(src_exts)[..., :3, -1:].transpose(-2,-1)  # (B, S, 1, 3)
    ...
    else: pts_mask = jt.empty(ray_diff_dot[..., :0].size())  # (B, S, P, 0)

    # Order matters, see how `ImageBasedRegressor` in `image_based_regressor.py` splits rgb from it
    img_feats = jt.cat([pts_mask, pts_feat, pts_inps, ray_diff_dir, ray_diff_dot], dim=-1)
...
def fused_mean_variance(x, weight):
    mean = jt.sum(x * weight, dim=-2, keepdim=True)
    var = jt.sum(weight * (x - mean) ** 2, dim=-2, keepdim=True)
    return mean, var
...
class FeatureAggIBRNet(nn.Module):
    def __init__(self, feat_ch, use_mvs_dens,
        ...
        if self.anti_alias_pooling:
            self.s = nn.Parameter(jt.Var(0.2), requires_grad=True)
    ...
    def execute(self, img_feat_rgb_dir: jt.Var):
        ...
        if self.anti_alias_pooling:
            exp_dot_prod = jt.exp(jt.abs(self.s) * (img_feat_rgb_dir[..., -1:] - 1))  # (B, P, S, 1)
            weight = (exp_dot_prod - jt.min(exp_dot_prod, dim=-2, keepdim=True)) * mask  # (B, P, S, 1)
            weight = weight / (jt.sum(weight, dim=-2, keepdim=True) + 1e-8)  # (B, P, S, 1)
        else:
            weight = mask / (jt.sum(mask, dim=-2, keepdim=True) + 1e-8)  # (B, P, S, 1)
        ...
        img_feat = jt.cat([jt.cat([avg_feat, var_feat], dim=-1).expand(-1, -1, S, -1), img_feat_rgb], dim=-1)  # (B, P, S, 3*Cf)
        ...
        res_feat, visibility = jt.split(vis_feat, [vis_feat.shape[-1] - 1, 1], dim=-1)  # (B, P, S, 32), (B, P, S, 1)
        ...
        app_feat = jt.cat([img_feat, visibility, img_feat_rgb_dir[..., 1 + self.feat_ch:]], dim=-1)  # (B, P, S, 32+1+4)
        ...
        if self.use_mvs_dens: density = None
            ...
            vis_weight = visibility / (jt.sum(visibility, dim=-2, keepdim=True) + 1e-8)  # (B, P, S, 1)
            avg_feat, var_feat = fused_mean_variance(img_feat, vis_weight)  # (B, P, 1, 32), (B, P, 1, 32)
            geo_feat = jt.cat([avg_feat.squeeze(-2), var_feat.squeeze(-2), vis_weight.mean(dim=-2)], dim=-1)  # (B, P, 32+32+1)
            geo_feat = self.geo_fc(geo_feat)  # (B, P, 16)
            density = self.out_geo_fc(geo_feat)  # (B, P, 1)
            # set the sigma of invalid point to zero
            density = density.masked_fill(jt.sum(mask, dim=-2) < 1, 0.)
