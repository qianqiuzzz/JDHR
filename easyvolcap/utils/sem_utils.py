...
def color_to_semantic(schp: jt.Var,  # B, H, W, 3
                      palette: jt.Var,  # 256, 3
                      ):
...
def semantics_to_color(semantic: jt.Var,  # V,
                       palette: jt.Var,  # 256, 3
                       ):
...
def palette_to_onehot(sem: np.ndarray, semantic_dim=semantic_dim):
    ...
    sem = jt.array(sem_msk)
    sem: jt.Var = nn.one_hot(sem.long(), semantic_dim)
    sem = sem.float().numpy()
    return sem
...
def get_schp_palette_tensor_float(num_cls=semantic_dim, device='cuda'):
    ...
    palette = jt.float32(palette) / 255.0
    palette = palette.reshape(-1, 3)  # n_cls, 3
    return palette
