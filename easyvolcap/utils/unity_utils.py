...
def unity_qt2opencv_w2c(quanternion: np.ndarray,
    ...
    Return:
        w2c: (jt.Var), (4, 4), world to camera matrix.
    """
...
def decode_single_unity_pose(quanternion: np.ndarray,
    ...
    # Convert the w2c into jt.Var
    w2c = jt.Var(w2c).float()  # (4, 4)
    return w2c
...
def decode_single_unity_proj_mats(k1, k2, k3, k4, H, W):
    ...
    Returns:
        K: (jt.Var), (3, 3), the camera intrinsic matrix of Easyvolcap.
    """
    fx = k1 * W / 2.
    cx = (1 - k2) * W / 2.
    fy = k3 * H / 2.
    cy = (k4 + 1) * H / 2.
    K = jt.Var([[fx, 0, cx], [0, fy, cy], [0, 0, 1]]).float()
    return K


def rectify_stereo_w2c_offset(w2cL: jt.Var, w2cR: jt.Var,
                              tL: List[float], tR: List[float]):
    """ Rectify the stereo offset for the two w2cs with same translation
    Args:
        w2cL: (jt.Var), (4, 4), the world2camera extrinsic matrix of the left camera.
        w2cR: (jt.Var), (4, 4), the world2camera extrinsic matrix of the right camera.
    ...
    Returns:
        w2cR: (jt.Var), (4, 4), rectified world2camera extrinsic matrix of the right camera.
    """
    c2w_offset = np.array(tR) - np.array(tL)  # (3,)
    c2w_offset = jt.Var(trans_unity2opencv(c2w_offset))[:, None].float()  # (3, 1)
...
def RT2c2w(R, T):
    ...
    Args:
        R: (jt.Var), (3, 3), rotation matrix.
        T: (jt.Var), (3, 1), translation matrix.
    Return:
        c2w: (jt.Var), (4, 4), camera to world matrix.
    """
    w2c = jt.eye(4)
    w2c[:3, :3], w2c[:3, 3:] = R, T
    c2w = jt.Var(np.inverse(w2c))
    return c2w


def encode_easyvolcap_stereo_imgs(tensor_left: jt.Var, tensor_right: jt.Var):
    """ Encode the easyvolcap rendered stereo image pairs using `turbojpeg`, which is
        ...
        right image.
    Args:
        tensor_left: (jt.Var), (B, H, W, 4), rendered left eye RGBA image, already detach and on cpu.
        tensor_right: (jt.Var), (B, H, W, 4), rendered right eye RGBA image, already detach and on cpu.
    Returns:
...
def log_stereo_params(w2cL, w2cR, KL, KR):
    ...
    Args:
        w2cL: (jt.Var), (4, 4), the world to camera matrix of the left eye
        w2cR: (jt.Var), (4, 4), the world to camera matrix of the right eye
        KL: (jt.Var), (3, 3), the camera intrinsic matrix of the left eye
        KR: (jt.Var), (3, 3), the camera intrinsic matrix of the right eye
    Returns:
        None
    """
