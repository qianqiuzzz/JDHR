...
def typed(input_to: jt.dtype = jt.float32, output_to: jt.dtype = jt.float32):
...
class VolumetricVideoModule(nn.Module):
    ...
    def __init__(self, network: nn.Module = None, **kwargs) -> None:
        ...
        # Prepare fake execute sample function
        # Hacky execute function definition
...
def make_params(params: jt.Var):
    return nn.Parameter(jt.array(params), requires_grad=True)


def make_buffer(params: jt.Var):
    return nn.Parameter(jt.array(params), requires_grad=False)


def take_jacobian(func: Callable, input: jt.Var, create_graph=False, vectorize=True, strategy='reverse-mode'):
    return 0#torch.autograd.functional.jacobian(func, input, create_graph=create_graph, vectorize=vectorize, strategy=strategy)#????


def take_gradient(output: jt.Var,
                  input: jt.Var,
                  d_out: jt.Var = None,
    ...
    if d_out is not None:
        d_output = d_out
    elif isinstance(output, jt.Var):
        d_output = jt.ones_like(output)
    else:
        d_output = [jt.ones_like(o) for o in output]
...
class MLP(GradientModule):
    def __init__(self, input_ch=32, W=256, D=8, out_ch=257, skips=[4], actvn=nn.ReLU(), out_actvn=nn.Identity(),
                 init_weight=nn.Identity(), init_bias=nn.Identity(), init_out_weight=nn.Identity(), init_out_bias=nn.Identity(), dtype=jt.float32):
        ...
        for i, l in enumerate(self.linears):
            if i == len(self.linears) - 1: init_out_weight(l.weight)
            else: init_weight(l.weight)

        for i, l in enumerate(self.linears):
            if i == len(self.linears) - 1: init_out_bias(l.bias)
            else: init_bias(l.bias)

    def execute_with_previous(self, input: jt.Var):
        x = input
        #print("input",self.out_actvn)
        for i, l in enumerate(self.linears):
            p = x  # store output of previous layer
            if i in self.skips:
                x = jt.cat([x, input], dim=-1)
            if i == len(self.linears) - 1:
                a = self.out_actvn
            else:
                a = self.actvn
            #print("i",i)
            x = a(l(x))  # actual execute
        return x, p

    def execute(self, input: jt.Var):
        return self.execute_with_previous(input)[0]
...
def update_optimizer_state(optimizer, optimizer_state):
    for k, v in optimizer_state.items():
        ...
        exp_avg = jt.zeros_like(v.new_params)
        exp_avg[v.new_keep] = val['exp_avg'][v.old_keep]
        val['exp_avg'] = exp_avg
        exp_avg_sq = jt.zeros_like(v.new_params)
