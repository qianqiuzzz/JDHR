...
def get_src_feats(src_inps: jt.Var, feat_reg: nn.Module, batch: dotdict, key: str = 'fg'):  # !: BATCH
    ...
    for i, v in enumerate(src_inds):
        ...
        feats.append(fs)
    feats = [jt.stack([f[i] for f in feats])[None] for i in range(len(feat_reg.scales))]  # HACK: too hacky...
...
def compute_src_inps(batch: dotdict, key: str = 'src_inps'):
    ...
    if key not in batch and key in batch.meta and isinstance(batch.meta[key], list) and batch.meta[key][0].ndim == 2:
        ...
        batch[key] = [jt.cat([decode_jpeg(inp.cpu(), device='cuda') for inp in inps])[None].float() / 255 for inps in batch.meta[key]]  # decode -> 3, H, W, uint8(0,255) -> float32(0,1)
    ...
    if isinstance(batch[key], list) and batch[key][0].ndim == 4:  # S: B, C, H, W -> B, S, C, H, W
        ...
        batch[key] = jt.stack([pad_image(img, [max_h, max_w]) for img in batch[key]]).permute(1, 0, 2, 3, 4)  # S: B, C, H, W -> B, S, C, H, W

    return batch[key].contiguous()  # always return a contiguous tensor


def compute_src_feats(src_inps: jt.Var, feat_reg: nn.Module, batch: dotdict = None):
...
class TruncatedExponential(Function):  # pylint: disable=abstract-method
    ...
    def backward(ctx, g):  # pylint: disable=arguments-differ
        ...
        return g * jt.exp(jt.clamp(x, min=-15, max=15))


trunc_exp = TruncatedExponential.apply
def trunc_exp_actvn(x: jt.Var): return trunc_exp(x - 1)
...
def sample_geometry_feature_image(xyz: jt.Var,  # B, P, 3
                                  src_feat_rgb: jt.Var,  # B, S, C, H, W
                                  src_exts: jt.Var,  # B, S, 4, 4
                                  src_ixts: jt.Var,  # B, S, 3, 3
                                  src_scale: jt.Var,
                                  padding_mode: str = 'border',

                                  #   sample_msk: bool = False,
                                  #   src_size: jt.Var = None,  # S, 2
                                  ):
    #print("sample_geometry_feature_image",xyz.shape,src_feat_rgb.shape,src_exts.shape,src_ixts.shape,src_scale.shape)
    # xyz: B, P, 3
    # src_feat_rgb: B, S, C, Hs, Ws
    B, S, C, Hs, Ws = src_feat_rgb.shape
    #print("src_feat_rgb",src_feat_rgb.shape)
    B, P, _ = xyz.shape
    #print("xyz",xyz.shape)
    xyz1 = jt.cat([xyz, jt.ones_like(xyz[..., -1:])], dim=-1)  # homogeneous coordinates
    #print("xyz1",xyz1.shape)
    src_ixts = src_ixts.clone()
    src_ixts[..., :2, :] *= src_scale
    #print("src_ixts",src_ixts.shape)
    # B, P, 4 -> B, 1, P, 4
    # B, S, 4, 4 -> B, S, 4, 4
    # -> B, S, P, 4
    #print("xyz1",xyz1[..., None, :, :].shape,src_exts.transpose(-2,-1).shape)
    xyz1 = jt.bmm_transpose(xyz1[..., None, :, :].repeat(1,S,1,1), src_exts)
    xyzs = jt.bmm_transpose(xyz1[..., :3] , src_ixts)  # B, S, P, 3 @ B, S, 3, 3
    xy = xyzs[..., :-1] / (xyzs[..., -1:] + 1e-8)  # B, S, P, 2
    x, y = xy.chunk(2, dim=-1)  # B, S, P, 1
    xy = jt.cat([x / Ws * 2 - 1, y / Hs * 2 - 1], dim=-1)  # B, S, P, 2
    #print("hahahah",(src_exts.transpose(-2,-1)).squeeze(2),(src_exts.transpose(-2,-1)).squeeze(2).shape)
    #xyz1 = jt.bmm(xyz1[..., None, :, :],src_exts.transpose(-2,-1))#(xyz1[..., None, :, :].bmm((src_exts.transpose(-2,-1)).squeeze(2)))
    #print("xyz1",xyz1.shape)
    #xyz1=xyz1.squeeze(2)
    #xyzs = jt.bmm(xyz1[..., :3],src_ixts.transpose(-2,-1))#xyz1[..., :3].bmm(src_ixts.transpose(-2,-1))  # B, S, P, 3 @ B, S, 3, 3
    ...
    #print("shape:",src_feat_rgb.view(-1, C, Hs, Ws).shape, xy.view(-1, 1, P, 2).shape)
    src_feat_rgb = nn.grid_sample(src_feat_rgb.view(-1, C, Hs, Ws), xy.view(-1, 1, P, 2), padding_mode=padding_mode).view(B, S, C, P).permute(0, 1, 3, 2)  # BS, C, 1, P -> B, S, C, P -> B, S, P, C
    return src_feat_rgb

    #if sample_msk:
    #    pixel_uv_range = jt.cat([x, y], dim=-1) / src_size[..., None, :] * 2 - 1  # B, S, P, 2 to sample the msk
...
def sample_feature_image(xyz: jt.Var,
                         src_feat_rgb: jt.Var,
                         tar_ext: jt.Var,
                         src_exts: jt.Var,
                         src_scale: jt.Var,
    ...
    src_feat_rgb = nn.grid_sample(src_feat_rgb.view(-1, C, Hs, Ws), xy.view(-1, 1, P, 2), padding_mode=padding_mode).view(B, S, C, P).permute(0, 1, 3, 2)
    ...
    src_feat_rgb_dir = jt.cat([src_feat_rgb, ray_diff_dir, ray_diff_dot], dim=-1)
    return src_feat_rgb_dir  # (B, S, P, C)



def sample_feature_volume(s_vals: jt.Var, feat_vol: jt.Var, ren_scale: jt.Var, tar_scale: jt.Var,
                          x: int = 0, y: int = 0, w: int = -1, h: int = -1):  # scale are in cpu to avoid sync
    # s_vals: B, P, N
    # feat_vol: B, C, D, Ht, Wt
    B, P, N = s_vals.shape
    B, C, D, Ht, Wt = feat_vol.shape
    WrHr = jt.array([Wt, Ht])[..., None] / tar_scale * ren_scale
    Wr, Hr = WrHr[0], WrHr[1]
    ...
    uv = create_meshgrid(Hr, Wr,dtype=s_vals.dtype, ndc=True).flip(-1)[y:y + h, x:x + w]  # xy
    d = s_vals * 2 - 1  # normalize to -1, 1, B, H, W, N
    uv = uv[None, ..., None, :].expand(B, -1, -1, N, -1)
    d = d.view(B, h, w, N)[..., None]  # B, H, W, N, 1
    uvd = jt.cat([uv, d], dim=-1)  # B, H, W, N, 3

    # B, C, D, H, W
    # B, 3, D, H, W
    vox_feat = nn.grid_sample(feat_vol, uvd, padding_mode='border')  # B, C, H, W, N
    vox_feat = vox_feat.permute(0, 2, 3, 4, 1)  # B, H, W, H, C
    return vox_feat.view(B, P * N, -1)  # B, P, C



def depth_regression(depth_prob: jt.Var, depth_values: jt.Var, volume_render_depth: bool = False, use_dist: bool = False) -> Tuple[jt.Var, jt.Var]:
    # depth_prob: B, D, H, W
    ...
    if volume_render_depth:
        ...
        if use_dist:
            ...
            occ = 1. - jt.exp(-raws * dists)  # B, HW, D
        else:
            occ = 1. - jt.exp(-raws)  # B, HW, D
        weights = render_weights(occ)  # B, HW, D
        acc_map = jt.sum(weights, -1, keepdim=True)  # (B, HW, 1)
        depth = weighted_percentile(jt.cat([z_vals, z_vals.max(dim=-1, keepdim=True)[0]], dim=-1),
                                    jt.cat([weights, 1 - acc_map], dim=-1), [0.45, 0.5, 0.55])  # B, HW, 3
...
def weight_regression(depth_prob: jt.Var, depth_values: jt.Var = None, use_dist: bool = False):
    B, D, H, W = depth_prob.shape
...
def build_cost_vol(depth: jt.Var, feat: jt.Var, mats: jt.Var, agg_method: str = 'var', correct_pix: bool = True):
    ...
    ref_grid = create_meshgrid(Ht, Wt,correct_pix=correct_pix, dtype=depth.dtype).flip(-1)  # (H, W, 2), in ij ordering
    ref_grid = jt.cat((ref_grid, jt.ones_like(ref_grid[..., -1:])), -1)  # H, W, 3
    ...
    if correct_pix: src_grid = jt.cat([src_grid[..., :1, :] / Ws * 2 - 1, src_grid[..., 1:, :] / Hs * 2 - 1], dim=-2)  # x, y
    else: src_grid = jt.cat([src_grid[..., :1, :] / (Ws - 1) * 2 - 1, src_grid[..., 1:, :] / (Hs - 1) * 2 - 1], dim=-2)  # x, y
...
def build_cost_vol_layer(depth: jt.Var, feat: jt.Var, mats: jt.Var, xywh: List[int], agg_method: str = 'var', correct_pix: bool = True):
    """Cost volume construction, firs step of depth volume construction
    Args:
        depth: jt.Var, (B, D, Ht, Wt), sampled feature volume depth, maybe the uniform first layer or guided second layer
        feat: jt.Var, (B, S, C, Hs, Ws), source image features
        homo: jt.Var, (B, S, 3, 4), target to source projection matrices
        xywh: jt.Var, (B, 4), the projected 2d foreground object bounding box
    Returns:
        cost_volume: jt.Var, (B, C, D, Hb, Wb), cost volume corresponding to the foreground object bounding box
    """
    ...
    # Create grid from the ref frame
    ref_grid = create_meshgrid(Ht, Wt, correct_pix=correct_pix, dtype=depth.dtype).flip(-1)[y:y + h, x:x + w, :]  # (Hb, Wb, 2), in xy ordering, # !: batch = 1
    ref_grid = jt.cat((ref_grid, jt.ones_like(ref_grid[..., -1:])), -1)  # (Hb, Wb, 3)
    ...
    # Actual grid sample
    cost_volume = nn.grid_sample(feat, src_grid, padding_mode='border', align_corners=not correct_pix)  # (B, C, D, H*W)
...
def get_proj_mats(src_scale: jt.Var, tar_scale: jt.Var,
                  tar_ext: jt.Var, tar_ixt: jt.Var,
                  src_exts: jt.Var, src_ixts: jt.Var,
                  ):
    ...
    tar_ones = jt.zeros_like(tar_ext[..., -1:, :])  # B, 1, 4
    tar_ones[..., -1] = 1  # B, 1, 4
    tar_projs = jt.cat((tar_projs, tar_ones), dim=-2)  # B, 4, 4
    tar_projs_inv = jt.Var(np.inverse(tar_projs.float()).to(tar_projs.dtype))  # B, 4, 4
