...
def depth_confidence(depth_prob: jt.Var):
    depth_range = jt.arange(depth_prob.shape[-3], dtype=jt.float)  # ,
    ...
    depth_prob_sum4 = nn.pad(depth_prob_sum4, pad=(0, 0, 0, 0, 1, 2))  # B, 1, D + 3, H, W
    depth_prob_sum4 = nn.AvgPool3d(depth_prob_sum4, (4, 1, 1), stride=1, padding=0)[:, 0]  # B, D, H, W
...
def depth2xyz(depth: jt.Var, ixts: jt.Var, exts: jt.Var, correct_pix: bool = True):
    ...
    ref_grid = create_meshgrid(H, W, correct_pix=correct_pix, dtype=depth.dtype).flip(-1)  # (H, W, 2), in ij ordering
    ref_grid = jt.cat([ref_grid, jt.ones_like(ref_grid[..., :1])], dim=-1)  # (H, W, 3)
...
def normalize_depth(depth: jt.Var, p: float = 0.01):
    n = int(depth.numpy().size * p)
    near = depth.reshape(-1,).topk(n, largest=False)[0].max()  # a simple version of percentile
    far = depth.reshape(-1,).topk(n, largest=True)[0].min()  # a simple version of percentile
    depth = 1 - (depth - near) / (far - near)
    depth = depth.clamp(0, 1)
    return depth


def depth_curve_fn(depth: jt.Var, p: float = 0.01, cm: str = 'linear'):
    depth = normalize_depth(depth)
    depth = colormap(depth, cm)
    return depth


def bisection(d_low: jt.Var, d_high: jt.Var, n_steps: int, ray_o: jt.Var, ray_d: jt.Var,
              decoder: nn.Module, batch: Mapping[str, jt.Var], tau: float):
...
def secant(f_low: jt.Var, f_high: jt.Var, d_low: jt.Var, d_high: jt.Var,
           n_steps: int, ray_o: jt.Var, ray_d: jt.Var, decoder: nn.Module,
           batch: Mapping[str, jt.Var], tau: float):
...
def ray_marching(ray_o: jt.Var, ray_d: jt.Var, decoder: nn.Module,
                 batch: Mapping[str, jt.Var],
    ...
    t_vals = jt.linspace(0, 1, steps=n_coarse_steps).view(1, 1, n_coarse_steps)
    ...
    n = n_batch * n_rays
    d_low = z_vals.view(n, n_coarse_steps, 1)[jt.arange(n), indices.view(n)].view(n_batch, n_rays)[mask]
    f_low = val.view(n, n_coarse_steps, 1)[jt.arange(n), indices.view(n)].view(n_batch, n_rays)[mask]
    indices = jt.clamp(indices + 1, max=n_coarse_steps - 1)
    d_high = z_vals.view(n, n_coarse_steps, 1)[jt.arange(n), indices.view(n)].view(n_batch, n_rays)[mask]
    f_high = val.view(n, n_coarse_steps, 1)[jt.arange(n), indices.view(n)].view(n_batch, n_rays)[mask]
    ...
    if method == 'secant' and mask.sum() > 0:
        ...
        d_pred = jt.ones(ray_d.shape[0])

    # for sanity
    d_out = jt.ones(n_batch, n_rays)
    d_out[mask] = d_pred

    return d_out, mask


def get_mask_from_occ(val: jt.Var):
    ...
    # last dimension
    sign_matrix = jt.cat([nn.sign(val[:, :, :-1] * val[:, :, 1:]), jt.ones(n_batch, n_rays, 1)], dim=-1)
    cost_matrix = sign_matrix * jt.arange(n_coarse_steps, 0, -1, dtype=jt.float32)
    ...
    values, indices = jt.argmin(cost_matrix, -1)
    mask_sign_change = values < 0
    mask_neg_to_pos = val[jt.arange(n_batch), jt.arange(n_rays), indices] < 0

    # Define mask where a valid depth value is found
    mask: jt.Var = mask_sign_change & mask_neg_to_pos & mask_0_not_occupied
