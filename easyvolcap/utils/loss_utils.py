...
def lpips(x: jt.Var, y: jt.Var):
    ...
    if not hasattr(lpips, 'compute_lpips'):
        from easyvolcap.utils.lpips.lpips import LPIPS
...
def ssim(x: jt.Var, y: jt.Var):
    ...
    from easyvolcap.utils.msssim_utils import _SSIMForMultiScale
    x_np=x.numpy()
    y_np=y.numpy()
...
def msssim(x: jt.Var, y: jt.Var):
    ...
    from easyvolcap.utils.msssim_utils import MultiScaleSSIM
    x_np=x.numpy()
    y_np=y.numpy()
