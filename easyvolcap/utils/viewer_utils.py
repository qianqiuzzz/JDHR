...
class CameraPath:
    ...
    def interp(self, us: float, **kwargs):
        ...
        K = jt.array(lin[:9]).view(3, 3)  # need a transpose
        H = int(lin[9])
        W = int(lin[10])
        n = jt.float(lin[11])
        f = jt.float(lin[12])
        t = jt.float(lin[13])
        v = jt.float(lin[14])
        bounds = jt.array(lin[15:]).view(2, 3)  # no need for transpose

        # Extract splined parameters
        w2c = affine_inverse(jt.array(c2w))  # already float32
        R = w2c[:3, :3]
        T = w2c[:3, 3:]
...
def lookat_bounds(bounds: List[List[float]], world_up: List[float], camera_center: Optional[List[float]] = None):
    ...
    bounds = jt.array(bounds, dtype=jt.float)  # 2, 3
    scene_center = bounds.mean(0)  # 3,

    if camera_center is None:
        camera_center = (bounds[0] - scene_center) * 1.5 + scene_center  # 3,

    world_up = jt.array(world_up, dtype=jt.float)  # 3,
    c2w = jt.eye(4)
    c2w[:3, 3] = camera_center
    z = normalize(scene_center - camera_center)
    x = jt.cross(z, world_up)
    y = jt.cross(z, x)
    c2w[:3, :3] = jt.stack([x, y, z]).transpose(-2,-1)  # 3, 3
...
class Camera:
    ...
    def __init__(self,
                 H: int = 768,
                 W: int = 1366,
                 K: jt.Var = jt.Var([[1366.0, 0.0, 683], [0.0, 1366.0, 384.0], [0.0, 0.0, 1.0]]),  # intrinsics
                 R: jt.Var = jt.Var([[-0.9977766275405884, 0.06664637476205826, 0.0], [0.004728599451482296, 0.07079283893108368, -0.9974799156188965], [-0.0664784237742424, -0.9952622056007385, -0.07095059007406235]]),  # extrinsics
                 T: jt.Var = jt.Var([[-2.059340476989746e-5], [2.5779008865356445e-6], [-3.000047445297241]]),  # extrinsics
                 ...
                 bounds: jt.Var = jt.Var([[-100.0, -100.0, -100.0], [100.0, 100.0, 100.0]]),  # bounding box

                 # camera update hyperparameters
                 origin: jt.Var = jt.Var([0.0, 0.0, 0.0]),
                 world_up: jt.Var = jt.Var([0.0, 0.0, 1.0]),
        ...
        if string is None:
            ...
            # Other configurables
            self.origin = vec3(*origin.data)
            # self.origin = self.center  # rotate about center
            self.world_up = vec3(*world_up.data)
    ...
    def to_batch(self):
        meta = dotdict()
        meta.H = jt.array(self.H)
        meta.W = jt.array(self.W)
        meta.K = jt.float(self.K.to_list()).transpose(-2,-1)
        meta.R = jt.float(self.R.to_list()).transpose(-2,-1)
        meta.T = jt.float(self.T.to_list())[..., None]
        meta.n = jt.float(self.n)
        meta.f = jt.float(self.f)
        meta.t = jt.float(self.t)
        meta.v = jt.float(self.v)
        meta.bounds = jt.float(self.bounds.to_list())  # no transpose for bounds

        # GUI related elements
        meta.mass = jt.float(self.mass)
        meta.moment_of_inertia = jt.float(self.moment_of_inertia)
        meta.movement_force = jt.float(self.movement_force)
        meta.movement_torque = jt.float(self.movement_torque)
        meta.movement_speed = jt.float(self.movement_speed)
        meta.origin = jt.float(self.origin.to_list())
        meta.world_up = jt.float(self.world_up.to_list())
    ...
    def from_batch(self, batch: dotdict):
        ...
        self.H = int(H)
        self.W = int(W)
        self.K = mat3(*K.transpose(-2,-1).data.flatten())
        self.R = mat3(*R.transpose(-2,-1).data.flatten())
        self.T = vec3(*T.data.flatten())  # 3,
        self.n = float(n)
        self.f = float(f)
        self.t = float(t)
        self.v = float(v)
        self.bounds = mat2x3(*bounds.data.flatten())  # 2, 3
    ...
    def custom_pose(self, R: jt.Var, T: jt.Var, K: jt.Var):
        # self.K = mat3(*K.mT.ravel())
        self.R = mat3(*R.transpose(-2,-1).reshape(-1,))
        self.T = vec3(*T.reshape(-1,))
