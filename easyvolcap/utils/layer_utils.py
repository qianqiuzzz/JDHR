...
def pad_layer_output(layer_output: dotdict):
    ...
    for key, value in layer_output.items():
        ...
        value_c = jt.zeros(B, H, W, N, C).to(value[-1].device)  # (B, H, W, N, C)
...
def pad_layers_output(layer_output: dotdict):
    ...
    for key, value in layer_output.items():
        ...
        for i in range(len(value)):
            ...
            # Actual complete logic
            value_c = jt.zeros(B, H, W, N, C).to(value[i].device)  # (B, H, W, N, C)
            value_c[:, y:y + h, x:x + w] = value[i].reshape(B, h, w, N, C)  # (B, H, W, N, C)
            output[key].append(value_c.reshape(B, -1, N, C))

        # Concatenate j layers output together
        output[key] = jt.cat(output[key], dim=-2)  # (B, H * W, N * num_fg_layer, C)

    # Sort the z_vals and get the indices
    output.z_vals, indices = jt.sort(output.z_vals, dim=-2)  # (B, H * W, N * num_fg_layer, 1)
    # Rearrange the other outputs according to the indices
    for key, value in output.items():
        if key in ['z_vals', 'HrWr', 'xywh', 'layer_render']: continue
        else: output[key] = jt.gather(value, dim=-2, index=indices.expand(-1, -1, -1, value.shape[-1]))

    return output


def volume_rendering_layer(output: dotdict, rgb: jt.Var, occ: jt.Var, bg_brightness=0.0):
    # See volume_rendering for more intuition
    # Concatenate the background rgb and occ to the last
    rgbs = jt.cat([output.rgb, rgb], dim=-2)  # (B, H * W, N * num_fg_layer + N, 3)
    occs = jt.cat([output.occ, occ], dim=-2)  # (B, H * W, N * num_fg_layer + N, 3)
