...
def get_proj_mats(src_scale: Union[List, jt.Var], tar_scale: Union[List, jt.Var],
    ...
    if isinstance(src_scale, List): src_scale = jt.float32(src_scale)[..., None]
    if isinstance(tar_scale, List): tar_scale = jt.float32(tar_scale)[..., None]
    ...
    tar_projs_inv = jt.linalg.inv(tar_projs)  # (B, 4, 4)
...
def get_rela_mats(tar_ext: jt.Var, src_ext: jt.Var):
    return tar_ext @ jt.linalg.inv(src_ext)  # (B, 4, 4)


def divide_safe_torch(num, den):
    # use torch.finfo to avoid divide by zero
    eps = misc.finfo(num.dtype).eps
    # substitute 0 with eps
    den = den + eps * (den == 0).float()
    return jt.div(num, den)


def get_inverse_homography(src_ixt: jt.Var, tar_ixt: jt.Var,
                           R: jt.Var, t: jt.Var,
                           n_hat: jt.Var, a: jt.Var):
    """Get inverse homography matrix from target image plane to source camera depth planes.
    Args:
        src_ixt (jt.Var): (D, B, 3, 3), intrinsic matrix of source camera.
        tar_ixt (jt.Var): (D, B, 3, 3), intrinsic matrix of target camera.
        R (jt.Var): (D, B, 3, 3), rotation matrix from source camera to target camera.
        t (jt.Var): (D, B, 3, 1), translation vector from source camera to target camera.
        n_hat (jt.Var): (D, B, 1, 3), normal vector of the plane.
        a (jt.Var): (D, B, 1, 1), depth of the plane.
    Returns:
        inv_homo (jt.Var): (D, B, 3, 3), homography matrix from source camera to target camera.
    """
    # See section 3.3 in https://arxiv.org/abs/1805.09817 for more detail
    R_trans = R.transpose(-2, -1)  # (D, B, 3, 3)
    denom = a - jt.matmul(jt.matmul(n_hat, R_trans), t)  # (D, B, 1, 1)
    numer = jt.matmul(jt.matmul(jt.matmul(R_trans, t), n_hat), R_trans)  # (D, B, 3, 3)
    # Compute the final inverse homography transformation matrix
    inv_homo = jt.matmul(src_ixt, jt.matmul(R_trans + divide_safe_torch(numer, denom), jt.linalg.inv(tar_ixt)))
    return inv_homo


def inverse_warpping(depth: jt.Var, mats: jt.Var, tar_ixt: jt.Var, src_ixt: jt.Var):
    """Inverse warp target image pixels to reference sweep volume planes.
    Args:
        depth (jt.Var): (B, D, H, W), sampled sweep volume depth of each plane, in reference camera coordinate.
        mats (jt.Var): (B, 4, 4), relative transformation matrix from reference camera coordinate to target camera coordinate.
        tar_ixt (jt.Var): (B, 3, 3), intrinsic matrix of target camera.
        src_ixt (jt.Var): (B, 3, 3), intrinsic matrix of reference camera.
    Returns:
        warpped_pts (jt.Var): (B, D, H * W, 2), the warpped target image pixels in reference image.
    """
    # Prepare shapes
    B, D, H, W = depth.shape
    ...
    tar_grid = jt.cat((tar_grid, jt.ones_like(tar_grid[..., -1:])), -1)  # (H, W, 3)
    tar_grid = tar_grid[None, None].expand(D, B, -1, -1, -1).view(D, B, -1, 3)  # (D, B, H * W, 3)

    # Transform the target grid to the source frame
    src_grid = jt.matmul(inverse_homography, tar_grid.transpose(-2, -1)).transpose(-2, -1)  # (D, B, H * W, 3)
    src_grid = src_grid[..., :-1] / src_grid[..., -1:].clip(1e-6)  # (D, B, H * W, 2)
    src_grid = jt.cat([src_grid[..., :1, :] / W * 2 - 1,  # x
...
def build_plane_sweep_volume(depth: jt.Var, feat: jt.Var, mats: jt.Var, agg_method=0):
    """Construct a source image plane sweep volume in reference image coordinate.
    Args:
        depth (jt.Var): (B, D, H, W), sampled sweep volume depth of each plane, in reference camera coordinate.
        feat (jt.Var): (B, S, C, H, W), original rgb image or extract image feature.
        mats (jt.Var): (B, S, 3, 4), projection matrices from reference image to source image.
        agg_method (int) or (str): different aggregation method.
    Returns:
        plane_sweep_volume (jt.Var): (B, C, D, H, W), a plane sweep volume.
    """
...
class LayerNorm(nn.Module):
    def __init__(self, out_channels, eps=1e-5, affine=True):
        ...
        if self.affine:
            self.weight = nn.Parameter(jt.Var(out_channels).uniform_())
...
class StereoMagnificationNet(nn.Module):
    ...
    def execute(self, x):
        ...
        x = jt.cat([out_conv4_3, out_conv3_3], dim=1)
        ...
        # Add skip connections
        x = jt.cat([out_conv5_3, out_conv2_2], dim=1)

        x = self.conv6_1(x)
        out_conv6_2 = self.conv6_2(x)

        # Add skip connections
        x = jt.cat([out_conv6_2, out_conv1_2], dim=1)
...
def raw2mpi(raw, batch):
    ...
    mpi_rgba = jt.cat([mpi_colors, mpi_alphas], dim=-1)  # (B, H, W, D, 4)
