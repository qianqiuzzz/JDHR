...
def geodesic_knn(pts: jt.Var,
    ...
    d2, nn = misc.knn(pts, verts, K)  # B, P, K; all valid index, not all valid points
    ...
    sdf_batch = dist_batch * jt.nn.sign(dot_batch)  # B, P, K, to match d2 return shape
    ...
    d2 = jt.ternary(msk, d2, d2[..., :1])  # fill invalid regions with closest
    nn = jt.ternary(msk, nn, nn[..., :1])

    tv = multi_gather(tverts[:, None].expand(B, P, N, -1), nn_batch)  # B, P, K, 3
    tv_cls = tv[..., :1, :]
    tv_to_tv_cls = (tv - tv_cls).pow(2).sum(-1)
    msk = tv_to_tv_cls < th ** 2
    sdf_batch = jt.ternary(msk, sdf_batch, sdf_batch[..., :1])
    nn_batch = jt.ternary(msk, nn_batch, nn_batch[..., :1])
...
class PointMeshDistance(Function):
    ...
    def forward(ctx, points, tris, n_batch=1):
        ...
        zeros = jt.zeros(n_batch, dtype=jt.int32)
        dists, idxs = _C.point_face_dist_forward(
            points, zeros, tris, zeros, n_points, 5e-3
        )
        ctx.save_for_backward(points, tris, idxs)
        return dists.sqrt(), idxs
    ...
    def backward(ctx, grad_dists):
        ...
        points, tris, idxs = ctx.saved_tensors
        grad_points, grad_tris = _C.point_face_dist_backward(
            points, tris, idxs, grad_dists, 5e-3
        )
...
def random_points_on_meshes_with_face_and_bary(meshes: Meshes, num_samples: int):
    ...
    num_valid_meshes = jt.sum(meshes.valid)  # Non empty meshes.

    # Initialize samples tensor with fill value 0 for empty meshes.
    samples = jt.zeros((num_meshes, num_samples, 3))
    ...
    bary = jt.stack((w0, w1, w2), dim=2)
    return samples, sample_face_idxs, bary


def get_voxel_grid_and_update_bounds(voxel_size: List, bounds: jt.Var):
    ...
    ret = []
    for b in bounds:
        x = jt.arange(b[0, 0].item(), b[1, 0].item() + voxel_size[0] / 2, voxel_size[0], dtype=b.dtype)
        y = jt.arange(b[0, 1].item(), b[1, 1].item() + voxel_size[1] / 2, voxel_size[1], dtype=b.dtype)
        z = jt.arange(b[0, 2].item(), b[1, 2].item() + voxel_size[2] / 2, voxel_size[2], dtype=b.dtype)
        pts = jt.stack(jt.meshgrid(x, y, z), dim=-1)
        ret.append(pts)
    pts = jt.stack(ret)  # dim 0
    bounds = jt.stack([pts[:, 0, 0, 0], pts[:, -1, -1, -1]], dim=1)  # dim 1 n_batch, 2, 3
    return pts, bounds


def optimize_until_no_nan(func: Callable[..., jt.Var], *args: jt.Var):
...
def optimze_samples_from_volume(grid: jt.Var, verts: jt.Var, faces: jt.Var, values: jt.Var, bounds: jt.Var, n_free_pts: int = 1024, n_surf_pts: int = 1024, norm_expand_step: int = 2, norm_expand_factor: float = 0.02, n_iter: int = 128, lr: float = 1e-3, clip_grad=10):
    ...
    p = tqdm(range(n_iter))
    with jt.enable_grad():  # you've turned off grad before main
        for _ in p:

            free_pts = jt.rand([n_batch, n_free_pts, 3], dtype=grid.dtype, device=grid.device)
            free_pts *= diag  # [0,1] -> diagonal
            free_pts += bounds[:, 0]  # diagonal -> shifted
            surf, norm = sample_points_from_meshes(mesh, n_surf_pts, True)
            surf_pts = jt.cat([surf + norm * norm_expand_factor * (jt.rand(1, dtype=grid.dtype) * 2 - 1) for _ in range(norm_expand_step)], dim=1)
            pts = jt.cat([free_pts, verts, surf_pts], dim=1)  # make sure verts gets mapped correctly
...
def batch_points_to_barycentric(points: jt.Var, triangles: jt.Var, method="cross", eps: float = 1e-16):
    return points_to_barycentric(points.view(-1, 3), triangles.view(-1, 3, 3), method, eps).view(*points.shape)


def points_to_barycentric(points: jt.Var, triangles: jt.Var, method="cross", eps: float = 1e-16):
    # points: n_points, 3
    # triangles: n_points, 3, 3

    def method_cross(edge_vectors: jt.Var, w: jt.Var):
        n = jt.cross(edge_vectors[:, 0], edge_vectors[:, 1])
        denominator = jt.bmm(n[:, None], n[..., None])[:, 0, 0]
        denominator[denominator.abs() < eps] = eps

        barycentric = jt.zeros((len(triangles), 3), dtype=points.dtype)
        barycentric[:, 2] = jt.bmm(jt.cross(edge_vectors[:, 0], w)[:, None], n[..., None])[:, 0, 0] / denominator  # u
        barycentric[:, 1] = jt.bmm(jt.cross(w, edge_vectors[:, 1])[:, None], n[..., None])[:, 0, 0] / denominator  # v
        barycentric[:, 0] = 1 - barycentric[:, 1] - barycentric[:, 2]  # w
        return barycentric

    def method_cramer(edge_vectors: jt.Var, w: jt.Var):
        # n_points, 1, 3 @ n_points, 3, 1 -> n_points, 1, 1
        dot00 = jt.bmm(edge_vectors[:, 0][:, None], edge_vectors[:, 0][..., None])[:, 0, 0]  # n_points
        dot01 = jt.bmm(edge_vectors[:, 0][:, None], edge_vectors[:, 1][..., None])[:, 0, 0]
        dot02 = jt.bmm(edge_vectors[:, 0][:, None], w[..., None])[:, 0, 0]
        dot11 = jt.bmm(edge_vectors[:, 1][:, None], edge_vectors[:, 1][..., None])[:, 0, 0]
        dot12 = jt.bmm(edge_vectors[:, 1][:, None], w[..., None])[:, 0, 0]
    ...
    def constraint_barycentric(barycentric: jt.Var):
...
def sample_texture(uvs: jt.Var, textures: jt.Var, from_ndc: bool = False, flip_tex: bool = True) -> jt.Var:
...
def sample_grid(pts: jt.Var, values: jt.Var, bounds: jt.Var):
...
def expand_points_for_sampling(verts: jt.Var, faces: jt.Var, values: jt.Var, expand_range: List, expand_step: int):
    ...
    # vert_norms = jt.Var(mesh.vertex_normals, dtype=pts.dtype, device=pts.device)[None] # add batch dimension
    expand_verts = jt.cat(
        [
...
def expand_verts_faces_along_normal(verts: jt.Var, faces: jt.Var, expand_range: List = [0.05, -0.05], expand_step: int = 5, randomize: bool = True):
    ...
    expanded_height: jt.Var = jt.arange(expand_step, dtype=verts.dtype)
    if randomize:
        expanded_height = expanded_height + jt.rand(expand_step, dtype=verts.dtype)
...
def sample_grid_closest_points(pts: jt.Var, verts: jt.Var, values: jt.Var):
    return [x.view(*pts.shape[:-1], -1) for x in sample_closest_points(pts.view(pts.shape[0], -1, 3), verts, values)]  # (1, whd, n_bones+3)


def sample_grid_closest_points_on_surface(pts: jt.Var, verts: jt.Var, faces: jt.Var, values: jt.Var):
...
def sample_grid_closest_points_on_expanded_surface(pts: jt.Var, verts: jt.Var, faces: jt.Var, values: jt.Var, expand_range: List, expand_step: int):
...
def cast_knn_points(src, ref, K=1):
    ret = misc.knn(src.float(), ref.float(), K)
...
def sample_closest_points(src: jt.Var, ref: jt.Var, values: jt.Var = None):
...
def sample_K_closest_points(src: jt.Var, ref: jt.Var, K: int = 4) -> Tuple[jt.Var, jt.Var]:
    # not so useful to aggregate all K points
    ret = cast_knn_points(src, ref, K=K)
    return ret[0], ret[1]  # (n_batch, n_points, K)


def sample_blend_K_closest_points(src: jt.Var, ref: jt.Var, values: jt.Var = None, K: int = 4, eps: float = 1e-8) -> Tuple[jt.Var, jt.Var]:
    # not so useful to aggregate all K points
    n_batch, n_points, _ = src.shape
    ret = cast_knn_points(src, ref, K=K)
    ...
    dists = jt.linalg.einsum('ijk,ijk->ij', dists, weights)
    if values is None:
        return dists.view(n_batch, n_points, 1)
    ...
    values = values.view(-1, values.shape[-1])  # (n, D)
    sampled = jt.linalg.einsum('ijkl,ijk->ijl', values[vert_ids], weights)
    return sampled.view(n_batch, n_points, -1), dists.view(n_batch, n_points, 1)


def sample_closest_points_on_surface_approx(points: jt.Var, verts: jt.Var, faces: jt.Var, values: jt.Var, n_surf_pts: int = 16384):
    # even slower...
    ...
    sampled = jt.sum(values[faces[face_ids]] *  # (n, 3, 3)
                        bary[..., None],  # (n, 3, 1)
                        dim=1)
...
def sample_closest_points_on_surface(points: jt.Var, verts: jt.Var, faces: jt.Var, values: jt.Var = None):
    ...
    # closest_verts = jt.Var(closest_verts).to(device)
    # barycentric = jt.Var(barycentric).to(device)
    # values = jt.sum(values[closest_verts] *  # (n, 3, 3)
    ...
    interp = jt.sum(values[faces[face_ids]] *  # (n, 3, 3)
                       bary[..., None],  # (n, 3, 1)
                       dim=1)
