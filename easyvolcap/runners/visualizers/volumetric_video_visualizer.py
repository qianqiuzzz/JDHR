...
class VolumetricVideoVisualizer:  # this should act as a base class for other types of visualizations (need diff dataset)
    def __init__(self,
                 ...
                 store_alpha_channel: bool = False,#True,  # store rendered acc in alpha channel
    ...
    def generate_type(self, output: dotdict, batch: dotdict, type: Visualization = Visualization.RENDER):
        ...
        img: jt.Var = None
        img_gt: Union[jt.Var, None] = None
        img_error: Union[jt.Var, None] = None

        if type == Visualization.NORMAL:
            def norm_curve_fn(norm):
                norm = normalize(norm)
                norm = norm @ batch.R.tranpose(-2,-1)
                norm[..., 1] *= -1
                norm[..., 2] *= -1
                norm = norm * 0.5 + 0.5
            ...
            def feat_curve_fn(feat: jt.Var):
                B, P, C = feat.shape
                N = C // 3  # ignore last few feature channels
                feat = jt.stack(feat[..., :3 * N].chunk(3, dim=-1), dim=-1).mean(dim=-2)  # now in rgb
                return feat
        ...
        # ... implement more
            ...
            img = jt.cat([img[:, i] for i in range(img.shape[1])], dim=-2)
        ...
        if self.store_image_error and img_gt is not None:
            img_error = (img - img_gt).pow(2).sum(dim=-1).clamp(0, 1)[..., None].expand(img.shape)

        if self.store_alpha_channel:
            msk = output.acc_map
            img = jt.cat([img, msk], dim=-1)
            if img_gt is not None:
                msk_gt = batch.msk
                img_gt = jt.cat([img_gt, msk_gt], dim=-1)
            if img_error is not None:
                msk_gt = batch.msk
                img_error = jt.cat([img_error, (msk_gt + msk).clamp(0, 1)], dim=-1)
        ...
        if self.uncrop_output_images:  # necessary for GUI applications
            if 'orig_h' in batch.meta:  # !: BATCH: Removed
                x, y, w, h = int(batch.meta.crop_x[0].item()), int(batch.meta.crop_y[0].item()), int(batch.meta.W[0].item()), int(batch.meta.H[0].item())
                H, W = int(batch.meta.orig_h[0].item()), int(batch.meta.orig_w[0].item())
    ...
    def visualize_type(self, output: dotdict, batch: dotdict, type: Visualization = Visualization.RENDER):
        ...
        image_stats = dotdict()
        camera_index: jt.Var = batch.meta.camera_index
        frame_index: jt.Var = batch.meta.frame_index
        img_paths = []
        img_arrays = []
        ...
        for i in range(len(imgs)):
            ...
            img_arrays.append(img.detach())  # start moving
            if img_gts is not None:
                img_paths.append(join(self.result_dir, img_gt_path))
                img_arrays.append(img_gt.detach())  # start moving
            if img_errors is not None:
                img_paths.append(join(self.result_dir, img_error_path))
                img_arrays.append(img_error.detach())  # start moving
                
            for j in range(len(img_arrays)):   
                save_image(img_path=img_paths[j], img=img_arrays[j])
