...
class GeometryVisualizer:
    ...
    def generate_type(self, output: dotdict, batch: dotdict, type: Visualization = Visualization.MESH):
        if type == Visualization.MESH:
            ...
            cube = jt.full((np.prod(batch.valid.shape),), -10.0, dtype=occ.dtype)[None]  # 1, WHD
            ...
            jt.sync_all(True)#torch.cuda.synchronize()  # some of the batched data are asynchronously moved to the cpu
            verts, faces = mcubes.marching_cubes(cube.float().numpy()[0], self.occ_thresh)
            verts = jt.array(verts, dtype=jt.float)[None]
            faces = jt.array(faces.astype(np.int32), dtype=jt.int)[None]
