    ...
    def render(self, rgb: jt.Var, occ: jt.Var, batch: dotdict):
        ...
        if self.training:
            if self.bg_brightness < 0:
                bg_color = jt.rand_like(rgb[..., 0, :])  # remove sample dim (in 0 -> 1 already)
            else:
                bg_color = jt.full_like(rgb[..., 0, :], self.bg_brightness)  # remove sample dim
        ...
        if isinstance(bg_color, jt.Var): output.bg_color = bg_color
        else: output.bg_color = jt.full_like(rgb_map, bg_color)  # B, P, 3
        ...
        def integrate(value: jt.Var, weights: jt.Var = weights, bg_color: jt.Var = bg_color):
        ...
        # Position output
            ...
            #    output.dpt_map = weighted_percentile(jt.cat([batch.output.z_vals, batch.output.z_vals.max(dim=-1, keepdim=True)], dim=-1),
            #                                         jt.cat([weights, 1 - acc_map], dim=-1), [0.5])
            #else:
            #    output.dpt_map = integrate(batch.output.z_vals, bg_color=batch.output.z_vals.max(dim=-1, keepdim=True))  # without last dim
