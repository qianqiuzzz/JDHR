...
class R4DVBSampler(R4DVSampler):
    ...
    def execute(self, batch: dotdict):
        ...
        batch.latent_index = jt.zeros_like(batch.latent_index)
        batch.meta.latent_index = jt.zeros_like(batch.meta.latent_index)
        ...
        pcd, xyz, rgb, rad, occ = [
            jt.cat([fg_pcd, bg_pcd], dim=-2),
            jt.cat([fg_xyz, bg_xyz], dim=-2),
            jt.cat([fg_rgb, bg_rgb], dim=-2),
            jt.cat([fg_rad, bg_rad], dim=-2),
            jt.cat([fg_occ, bg_occ], dim=-2)]
        ...
        if 'msk' in batch:
            batch.msk = jt.ones_like(batch.msk)  # gatekeeping for training
