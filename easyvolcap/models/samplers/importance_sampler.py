    ...
    def sample_depth(self,
        ...
        for round in range(len(self.n_samples)):
            ...
            if round == 0:
                ...
                s_vals = linear_sampling(B, P, n_samples, dtype=ray_o.dtype, perturb=self.training)  # 0 -> B, P, S
            ...
            if round != len(self.n_samples) - 1:
                ...
                if self.normalize_weight:
                    weights = weights / (weights.sum(dim=-1, keepdim=True) + jt.misc.finfo("float").eps)
        ...
        if self.n_freespace > 0 and self.training:  # only add in these perturbations during training
            s_vals_rand = linear_sampling(B, P, self.n_freespace, dtype=ray_o.dtype, perturb=self.training)  # 0 -> B, P, S
            z_vals_rand = s_vals_to_z_vals(s_vals_rand, near, far, g=self.g, ig=self.ig)
            z_vals = jt.cat([z_vals, z_vals_rand], dim=-1).sort(dim=-1)[0]  # near to far
            s_vals = jt.cat([s_vals, s_vals_rand], dim=-1).sort(dim=-1)[0]  # near to far
