    ...
    def compute_src_feats(self, batch: dotdict):
        ...
        for i, inp in enumerate(src_inps[0]):  # !: BATCH
            if i < self.n_track:
                ...
                with jt.no_grad():  # no gradient tracking for these images, to save training time
                    feats.append(compute_src_feats(inp, self.feat_reg, batch)[-1])
        
        src_feat = jt.stack(feats)[None]  # S, C, H, W -> B, S, C, H, W
        src_feat_rgb = jt.cat([src_feat, src_inps], dim=-3)  # B, S, C, Hr, Wr
        
        batch.persistent.src_feat_rgb = src_feat_rgb
        batch.persistent.src_scale = src_scale

    def execute(self, xyz: jt.Var, batch: dotdict, optimize_cnn: bool = True):
        # xyz: B, P * S, 3
        if not optimize_cnn:
            with jt.no_grad():
        ...
        src_feat_rgb = sample_geometry_feature_image(xyz, src_feat_rgb, src_exts, src_ixts, src_scale)  # B, S, P, C  MARK:this function is right
