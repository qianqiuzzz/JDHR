...
class ImageBasedRegressor(nn.Module):
    ...
    def execute(self, geo_feat: jt.Var, batch: dotdict):
        ...
        if self.use_dir:
            src_feat: jt.Var = batch.output.src_feat_rgb_dir  # B, S, P, C
            src_rgbs = src_feat[..., -4 - 3:-4]  # -4: dir feat, -7 -> -3: rgb, B, S, P, 3
        else:
            src_feat: jt.Var = batch.output.src_feat_rgb
            src_rgbs = src_feat[..., -3:]
        ...
        if self.manual_chunking:
            ...
            for i in range(B):  # TODO: PERF
                for j in range(S):
                    feat = jt.cat([geo_feat[i], src_feat[i, j]], dim=-1)  # P, C
            ...
            geo_feat = jt.cat([geo_feat, src_feat], dim=-1)  # +7, append the actual image feature
