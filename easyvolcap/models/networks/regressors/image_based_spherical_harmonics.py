    ...
    def __init__(self,
        ...
        if not self.skip_shs:
            ...
            if resd_init is not None:
                ...
                [jt.init.gauss_(self.sh_mlp.mlp.linears[i].weight,0, resd_weight_init) for i in range(len(self.sh_mlp.mlp.linears))]
                [jt.init.fill(self.sh_mlp.mlp.linears[i].bias,resd_init if i == len(self.sh_mlp.mlp.linears) - 1 else -1) for i in range(len(self.sh_mlp.mlp.linears))]

    def execute(self, xyz_feat_dir: jt.Var, batch: dotdict, return_rgb_sh: bool = False):
        ...
        ibr_feat: jt.Var = batch.output.src_feat_rgb  # B, S, P, C
        ...
        if self.manual_chunking:
            ...
            for i in range(B):  # TODO: PERF
                for j in range(S):
                    feat = jt.cat([xyz_feat[i], ibr_feat[i, j]], dim=-1)  # P, C
                    rgb_bws.append(self.rgb_mlp(feat))  # P, 1
            rgb_bws = jt.stack(rgb_bws)  # BS, P, 1
            rgb_bws = rgb_bws.view(B, S, P, -1)  # B, S, P, 1
        else:
            exp_xyz_feat = xyz_feat[:, None].expand(ibr_feat.shape[:-1] + (xyz_feat.shape[-1], ))  # B, S, P, C
            feat = jt.cat([exp_xyz_feat, ibr_feat], dim=-1)  # +7, append the actual image feature
        ...
        if not self.skip_shs:
            if self.blend_shs:
                sh = self.sh_mlp(jt.cat([xyz_feat, ibr_feat.var(1), ibr_feat.mean(1)], dim=-1))  # only available for geometry rendering
            else:
        ...
        rgb = rgb.clamp(0, 1)  # maybe not needed?
        return rgb
