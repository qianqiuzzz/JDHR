...
class GeneralizableDataset(VolumetricVideoDataset):
    def __init__(self,
        ...
        self.lengths = jt.array([len(d) for d in self.datasets])
    ...
    def extract_dataset_index(self, index: Union[dotdict, int]):
        ...
        dataset_index = jt.searchsorted(self.accum_lengths, sampler_index, right=True)  # 2 will not be inserted as 2
        ...
        sampler_index = sampler_index.item() if isinstance(sampler_index, jt.Var) else sampler_index  # convert to int
