...
from easyvolcap.utils.math_utils import affine_inverse,affine_inverse_np, affine_padding, affine_padding_np,torch_inverse_3x3, point_padding,point_padding_np
from easyvolcap.utils.bound_utils import get_bound_2d_bound,get_bound_2d_bound_np, get_bounds,get_bounds_np, monotonic_near_far, monotonic_near_far_np,get_bound_3d_near_far
from easyvolcap.utils.data_utils import DataSplit, load_resize_undist_ims_bytes, load_image_from_bytes, as_torch_func, to_cuda, to_cpu, to_tensor, export_pts, load_pts, decode_crop_fill_ims_bytes, decode_fill_ims_bytes
...
class VolumetricVideoDataset(Dataset):
    def __init__(self,
        ...
        self.n_rays = np.array(n_rays,dtype=np.int32)
        self.patch_size = np.array(patch_size,dtype=np.int32)
        self.render_ratio = np.array(render_ratio,dtype=float)
        self.random_crop_size = np.array(random_crop_size,dtype=float)
        self.render_center_crop_ratio = np.array(render_center_crop_ratio,dtype=float)
    ...
    def load_paths(self):
        ...
        if self.split == DataSplit.TRAIN:
            self.rank = jt.rank if self.ddp_shard_dataset else 0  # 0 for non ddp
            self.num_replicas = jt.world_size if self.ddp_shard_dataset else 1  # 1 for non ddp
    ...
    def load_bytes(self):
        ...
        dist_mask = np.array(self.dist_mask)
        self.Ds = self.Ds.reshape(*self.Ds.shape[:2], 5) * dist_mask  # some of the distortion parameters might need some manual massaging
        ...
        if self.use_masks:
            self.mks_bytes, self.Ks, self.Hs, self.Ws = \
                load_resize_undist_ims_bytes(self.mks, ori_Ks, ori_Ds, ratio, self.center_crop_size,
        ...
        self.ims_bytes, self.Ks, self.Hs, self.Ws = \
            load_resize_undist_ims_bytes(self.ims, ori_Ks, ori_Ds, ratio, self.center_crop_size,
        ...
        if self.immask_crop:  # a little bit wasteful but acceptable for now
            ...
            bounds = np.tile(np.stack(bounds)[None],(self.n_views,1,1,1))  # V, N, 2, 3
            if hasattr(self, 'dps_bytes'): self.dps_bytes, mks_bytes, Ks, Hs, Ws, crop_xs, crop_ys = \
                decode_crop_fill_ims_bytes(self.dps_bytes, self.mks_bytes, self.Ks, self.Rs, self.Ts, bounds, f'Cropping msks dpts for {blue(self.data_root)} {magenta(self.split.name)}', encode_ext=['.exr', self.encode_ext])

            if hasattr(self, 'nms_bytes'): self.nms_bytes, mks_bytes, Ks, Hs, Ws, crop_xs, crop_ys = \
                decode_crop_fill_ims_bytes(self.nms_bytes, self.mks_bytes, self.Ks, self.Rs, self.Ts, bounds, f'Cropping msks nrms for {blue(self.data_root)} {magenta(self.split.name)}', encode_ext=self.encode_ext)

            self.ims_bytes, self.mks_bytes, self.Ks, self.Hs, self.Ws, self.crop_xs, self.crop_ys = \
                decode_crop_fill_ims_bytes(self.ims_bytes, self.mks_bytes, self.Ks, self.Rs, self.Ts, bounds, f'Cropping msks imgs for {blue(self.data_root)} {magenta(self.split.name)}', encode_ext=self.encode_ext)

            self.corp_xs = np.array(self.crop_xs)
            self.corp_ys = np.array(self.crop_ys)
            self.Ks = np.array(self.Ks)
            self.Hs = np.array(self.Hs)
            self.Ws = np.array(self.Ws)
    ...
    def load_vhulls(self):
        ...
        def carve_using_bytes(H, W, K, R, T, latent_index):
            ...
            msks_full = H.new_zeros(N, H_max, W_max, 1, dtype=jt.float32)
        ...
        Hs = [_ for _ in np.moveaxis(self.Hs,0, 1)]  # F, V,
        Ws = [_ for _ in np.moveaxis(self.Ws,0, 1)]
        Ks = [_ for _ in np.moveaxis(self.Ks,0, 1)]  # F, V, 3, 3
        Rs = [_ for _ in np.moveaxis(self.Rs,0, 1)]
        Ts = [_ for _ in np.moveaxis(self.Ts,0, 1)]
        ...
        for latent_index, (vh, H, W, K, R, T) in enumerate(tqdm(zip(self.vhs, Hs, Ws, Ks, Rs, Ts), total=len(Hs), desc=f'Preparing vhulls {blue(join(self.data_root, self.vhulls_dir))} {magenta(self.split.name)}')):
            ...
            if not exists(vh) or self.reload_vhulls:#??????
                ...
                if self.use_aligned_cameras:
                    ...
                    world_vhulls = (point_padding(vhulls) @ mat.swapaxes(-2,-1))  # homo
                    world_vhulls = world_vhulls[..., :3] / world_vhulls[..., 3:]
                else:
                    world_vhulls = vhulls
                export_pts(world_vhulls, filename=vh)
            else:
                world_vhulls = load_pts(vh)[0] # N, 3; 2, 3
                if self.use_aligned_cameras:
                    mat = affine_inverse_np(affine_padding_np(self.c2w_avg))  # 4, 4
                    vhulls = (point_padding_np(world_vhulls) @ mat.swapaxes(-2,-1))[..., :3]  # homo
                else:
                    vhulls = world_vhulls
                bounds = get_bounds_np(vhulls[None])[0]
        ...
        self.vhull_bounds = np.stack(bounds)  # F, 2, 3
        if self.print_vhull_bounds:
            ...
            log(magenta(f'Accumulated vhull bound of frames: {self.frame_sample}: '),
                line(jt.stack([self.vhull_bounds.argmin(dim=0)[0][0],
                                  self.vhull_bounds.argmax(dim=0)[0][1]])))
        if self.vhull_only:
            exit(0)
    ...
    def load_smpls(self):
        ...
        def get_lbs_params(i):
            ...
            if self.use_aligned_cameras:
                R = jt.array(cv2.Rodrigues(Rh[0].numpy())[0])  # 3, 3
                Rt = jt.cat([R, Th[0].view(3, 1)], dim=1)  # 3, 4
                Rt = (affine_inverse(affine_padding(self.c2w_avg)) @ affine_padding(Rt))[:3, :]  # 3, 4
                Rh = jt.array(cv2.Rodrigues(Rt[:, :-1].numpy())[0]).view(1, 3)  # 1, 3
        ...
        self.smpl_motions.poses = jt.cat(poses)
        self.smpl_motions.shapes = jt.cat(shapes)
        self.smpl_motions.Rh = jt.cat(Rh)
        self.smpl_motions.Th = jt.cat(Th)
    ...
    def load_cameras(self):
        ...
        self.Hs = np.array([[cam.H for cam in self.cameras[k]] for k in self.camera_names], dtype=np.int32)  # V, F
        self.Ws = np.array([[cam.W for cam in self.cameras[k]] for k in self.camera_names], dtype=np.int32)  # V, F
        self.Ks = np.array([[cam.K for cam in self.cameras[k]] for k in self.camera_names], dtype=float)  # V, F, 3, 3
        self.Rs = np.array([[cam.R for cam in self.cameras[k]] for k in self.camera_names], dtype=float)  # V, F, 3, 3
        self.Ts = np.array([[cam.T for cam in self.cameras[k]] for k in self.camera_names], dtype=float)  # V, F, 3, 1
        self.Ds = np.array([[cam.D for cam in self.cameras[k]] for k in self.camera_names], dtype=float)  # V, F, 1, 5
        self.ts = np.array([[cam.t for cam in self.cameras[k]] for k in self.camera_names], dtype=float)  # V, F
        self.ns = np.array([[cam.n for cam in self.cameras[k]] for k in self.camera_names], dtype=float)  # V, F
        self.fs = np.array([[cam.f for cam in self.cameras[k]] for k in self.camera_names], dtype=float)  # V, F
        self.Cs = -self.Rs.swapaxes(-2,-1) @ self.Ts  # V, F, 3, 1
        self.w2cs = np.concatenate([self.Rs, self.Ts], axis=-1)  # V, F, 3, 4
        self.c2ws = affine_inverse_np(self.w2cs)  # V, F, 3, 4
        self.ns, self.fs = monotonic_near_far_np(self.ns, self.fs,np.array(self.near, dtype=float), np.array(self.far, dtype=float))
    ...
    def align_cameras(self):
        ...
        self.c2ws = self.c2ws.reshape((-1,) + sh[-2:])  # V*F, 3, 4
        c2w_avg = average_c2ws(self.c2ws.reshape(sh)[:, 0])  # V, 3, 4
        self.c2w_avg = c2w_avg
        self.c2ws = np.matmul(affine_inverse_np(affine_padding_np(self.c2w_avg))[None],affine_padding_np(self.c2ws))[..., :3, :]  # 1, 4, 4 @ V*F, 4, 4 -> V*F, 3, 4
        self.w2cs = affine_inverse_np(self.c2ws)  # V*F, 3, 4
        self.c2ws = self.c2ws.reshape(sh)
        self.w2cs = self.w2cs.reshape(sh)
    ...
    def select_cameras(self):
        ...
        view_inds = np.arange(self.Ks.shape[0])
        ...
        frame_inds = np.arange(self.Ks.shape[1])
    ...
    def get_image(self, view_index: int, latent_index: int):
        ...
        if self.cache_raw:
            rgb = np.array(im_bytes)
        else:
            rgb = np.array(load_image_from_bytes(im_bytes, normalize=True)) # 4-5ms for 400 * 592 jpeg, sooo slow

        # Load mask from bytes
        if mk_bytes is not None:
            if self.cache_raw:
                msk = np.array(mk_bytes)
            else:
                msk = np.array(load_image_from_bytes(mk_bytes, normalize=True)[..., :1])
        else:
            msk = np.ones_like(rgb[..., -1:])

        # Load sampling weights from bytes
        if wt_bytes is not None:
            if self.cache_raw:
                wet = np.array(wt_bytes)
            else:
                wet = np.array(load_image_from_bytes(wt_bytes, normalize=True)[..., :1])
        else:
            wet = msk#.clone()
        ...
        if dp_bytes is not None:
            if self.cache_raw:
                dpt = np.array(dp_bytes)
            else:
                dpt = np.array(load_image_from_bytes(dp_bytes, normalize=False)[..., :1])  # readin as is

        # Load background image from bytes
        if bg_bytes is not None:
            bg_bytes = self.bgs_bytes[view_index]
            if self.cache_raw:
                bkg = np.array(bg_bytes)
            else:
                bkg = np.array(load_image_from_bytes(bg_bytes, normalize=True))

        # Load normal from bytes
        if nm_bytes is not None:
            if self.cache_raw:
                norm = np.array(nm_bytes)
            else:
                norm = np.array(load_image_from_bytes(nm_bytes, normalize=True)) # readin as is
    ...
    def get_objects_bounds(self, latent_index):
        ...
        elif self.objects_bounds is not None: bounds = jt.array(self.objects_bounds, dtype=jt.float)  # 2, 3
    ...
    def get_objects_priors(self, output: dotdict):
        ...
        objects_xywh.append(jt.array([x, y, w, h], dtype=jt.int32))
        objects_n.append(near)
        objects_f.append(far)

        meta = dotdict()
        meta.objects_bounds = jt.stack(to_tensor(objects_bounds), dim=0)  # (Nf, 2, 3)
        meta.objects_xywh = jt.stack(objects_xywh, dim=0)  # (Nf, 4)
        meta.objects_n = jt.array(objects_n, dtype=jt.float)  # (Nf,)
        meta.objects_f = jt.array(objects_f, dtype=jt.float)  # (Nf,)
    ...
    def get_metadata(self, index):
        ...
        meta.t = np.array(meta.t, dtype=float)  # the dataset provided time or the time fraction
        meta.v = self.camera_to_v(camera_index)
        meta.v = np.array(meta.v, dtype=float)  # the time fraction
        meta.n_rays = self.n_rays
        if self.use_aligned_cameras: meta.c2w_avg = self.c2w_avg  # MARK: store the aligned cameras here

        # Other inputs
        meta.bounds = self.get_bounds(latent_index)#.numpy()
    ...
    def scale_ixts(output: dotdict, ratio: float):
        ...
        meta.K = K
        meta.tar_ixt = K
        meta.H = np.array(new_h)
        meta.W = np.array(new_w)
        if 'orig_h' in output:
            meta.crop_x = np.array(int(output.crop_x * ratio))
            meta.crop_y = np.array(int(output.crop_y * ratio))  # TODO: this is messy
            meta.orig_h = np.array(int(output.orig_h * ratio))
            meta.orig_w = np.array(int(output.orig_w * ratio))
            ...
            meta.K[..., :2, -1] -= np.array([output.crop_x * ratio - meta.crop_x,  # only dealing with the cropped fraction
    ...
    def crop_ixts(output: dotdict, x, y, w, h):
        ...
        if 'crop_x' in output.meta:
            meta.crop_x = np.array(x + output.meta.crop_x)
            meta.crop_y = np.array(y + output.meta.crop_y)
        else:
            meta.crop_x = np.array(x)
            meta.crop_y = np.array(y)
    ...
    def get_ground_truth(self, index):
        ...
        render_ratio = render_ratio#.numpy()
        render_center_crop_ratio = render_center_crop_ratio#.numpy()
        ...
        try:
            if (len(render_ratio.shape) and  # avoid length of 0-d tensor error, check length of shape
                ...
                rgb = partial(cv2.resize, dsize=(W, H), interpolation=cv2.INTER_AREA)(rgb)
                msk = partial(cv2.resize, dsize=(W, H), interpolation=cv2.INTER_AREA)(msk)
                wet = partial(cv2.resize, dsize=(W, H), interpolation=cv2.INTER_AREA)(wet)
                if dpt is not None: partial(cv2.resize, dsize=(W, H), interpolation=cv2.INTER_AREA)(dpt)
                if bkg is not None: partial(cv2.resize, dsize=(W, H), interpolation=cv2.INTER_AREA)(bkg)
                if norm is not None: partial(cv2.resize, dsize=(W, H), interpolation=cv2.INTER_AREA)(norm)
        except:
            pass
        ...
        if should_crop_ixt:
            ...
            meta = dotdict()
            meta.patch_h = np.array(h)
            meta.patch_w = np.array(w)
            output.update(meta)
            output.meta.update(meta)
    ...
    def __getitem__(self, index: int):  # for now, we are using the dict as list
        ...
        if 'ray_o' in output or self.n_rays < 0: return output  # directly return for the whole image (same for train and test)#self.n_rays < 0#????
        ...
        rgb = output.rgb.reshape(H, W, 3)
        msk = output.msk.reshape(H, W, 1)
        wet = output.wet.reshape(H, W, 1)
        if 'dpt' in output: dpt = output.dpt.reshape(H, W, 1)
        if 'bkg' in output: bkg = output.bkg.reshape(H, W, 3)
        if 'norm' in output: norm = output.norm.reshape(H, W, 3)

        # Sample rays
        ray_o, ray_d, coords = weighted_sample_rays(wet,
                                                    K, R, T,
                                                    self.n_rays if self.split == DataSplit.TRAIN else -1,#增大n_rays
                                                    self.use_z_depth,
                                                    self.correct_pix)  # N, 3; N, 3; N, 3; N, 2 (100ms)

        # Access and fetch data
        
        i, j = coords.T[0],coords.T[1]
    ...
    def crop_ixts_bounds(output: dotdict):
        ...
        """
        x, y, w, h = get_bound_2d_bound_np(output.bounds, output.K, output.R, output.T, output.meta.H, output.meta.W)
        return VolumetricVideoDataset.crop_ixts(output, x, y, w, h)

    def get_viewer_batch(self, output: dotdict):#zhushi
        # Source indices
        t = output.t
        v = output.v
        ...
        output.bounds[0] = jt.maximum(output.bounds[0], bounds[0])  # crop according to user bound
