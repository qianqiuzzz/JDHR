...
class VolumetricVideoInferenceDataset(VolumetricVideoDataset):
    ...
    def load_size(self, size: List[int], overwrite: bool = False):
        ...
        if not overwrite:
            self.Ks = jt.stack([(k if h > 0 and w > 0 else K) for k, h, w in zip(self.Ks, self.Hs, self.Ws)])  # N, 3, 3
            self.Hs = jt.array([(h if h > 0 else H) for h in self.Hs])
            self.Ws = jt.array([(w if w > 0 else W) for w in self.Ws])
        else:
            self.Ks = K[None].expand(len(self.Ks), -1, -1)
            self.Hs = jt.full_like(self.Hs, H)
            self.Ws = jt.full_like(self.Ws, W)
    ...
    def interpolate_intrinsics(self):
        ...
        if self.interp_type != Interpolation.NONE:
            ...
            self.Ks = jt.array(interpolate_camera_lins(self.Ks.float().view(-1, 9).numpy(), self.n_render_views, **self.interp_cfg)).view(-1, 3, 3)
            self.Hs = jt.array(interpolate_camera_lins(self.Hs.float().view(-1, 1).numpy(), self.n_render_views, **self.interp_cfg)).view(-1).int()
            self.Ws = jt.array(interpolate_camera_lins(self.Ws.float().view(-1, 1).numpy(), self.n_render_views, **self.interp_cfg)).view(-1).int()
            self.ns = jt.array(interpolate_camera_lins(self.ns.float().view(-1, 1).numpy(), self.n_render_views, **self.interp_cfg)).view(-1)
            self.fs = jt.array(interpolate_camera_lins(self.fs.float().view(-1, 1).numpy(), self.n_render_views, **self.interp_cfg)).view(-1)
    ...
    def interpolate_times(self):
        ...
        if self.temporal_range_overwrite is not None and len(self.temporal_range_overwrite):
            ts = jt.array(self.temporal_range_overwrite).float()  # 2, will this work?
        else:
        ...
        if len(ts) > 1: self.ts = jt.array(interpolate_camera_lins(ts.view(-1, 1).numpy(), self.n_render_views, **self.interp_cfg)).view(-1)[:, None].expand(-1, self.n_latents)  # (V, F)
        else: self.ts = jt.array(ts).repeat(self.n_render_views)[:, None].expand(-1, self.n_latents)  # 100, (V, F)
