...
import easyvolcap.structures.utils as struct_utils
...
class Pointclouds:
    ...
    def __init__(self, points, normals=None, features=None) -> None:
        ...
        if isinstance(points, list):
            ...
            self.valid = jt.zeros((self._N,)).bool()
            ...
            if self._N > 0:
                ...
                self._num_points_per_cloud = jt.array([], dtype=jt.int64)
            ...
            self.valid = jt.ones((self._N,)).bool()
    ...
    def _parse_auxiliary_input_list(
        ...
        if needs_fixing:
            if good_empty is None:
                good_empty = jt.zeros((0, aux_input_C))
            aux_input_out = []
            for p, d in zip(self._num_points_per_cloud, aux_input):
                valid_but_empty = p == 0 and d is not None and d.ndim == 2
                if p > 0 or valid_but_empty:
    ...
    def __getitem__(
        self,
        index: Union[int, List[int], slice, jt.bool, jt.int64],
    ) -> "Pointclouds":
        """
        Args:
        ...
        elif isinstance(index, jt.Var):
            if index.dim() != 1 or index.dtype.is_float():
                raise IndexError(index)
            # NOTE consider converting index to cpu for efficiency
            if index.dtype == jt.bool:
                # advanced indexing on a single dimension
                index = index.nonzero()
                index = index.squeeze(1) if index.numpy().size > 0 else index
                index = index.tolist()
            points = [self.points_list()[i] for i in index]
            if normals_list is not None:
                normals = [normals_list[i] for i in index]
    ...
    def normals_packed(self) -> Optional[jt.Var]:
        """
    ...
    def normals_padded(self) -> Optional[jt.Var]:
        """
    ...
    def features_padded(self) -> Optional[jt.Var]:
        """
    ...
    def padded_to_packed_idx(self):
        ...
        if self._N == 0:
            ...
            self._padded_to_packed_idx = jt.cat(
                [
                    jt.arange(v, dtype=jt.int64) + i * self._P
    ...
    def _compute_padded(self, refresh: bool = False):
        ...
        if self.isempty():
            self._points_padded = jt.zeros((self._N, 0, 3))
        else:
    ...
    def _compute_packed(self, refresh: bool = False):
        ...
        if not np.allclose(self._num_points_per_cloud.numpy(), points_list_to_packed[1].numpy()):
    ...
    def to(self, device, copy: jt.bool = False):
        """
        Match functionality of jt.Var.to()
    ...
    def subsample(self, max_points: Union[int, Sequence[int]]) -> "Pointclouds":
        ...
        for max_, n_points, points, features, normals in zip_longest(
            ...
            if n_points > max_:
                ...
                keep = jt.array(keep_np,dtype=jt.int64)
                points = points[keep]
    ...
    def scale_(self, scale):
        ...
        Returns:
            self.
        """
        if not jt.is_var(scale):
            scale = jt.full((len(self),), scale)
        new_points_list = []
        points_list = self.points_list()
        ...
        if self._points_packed is not None:
            self._points_packed = jt.cat(new_points_list, dim=0)
        if self._points_padded is not None:
            for i, points in enumerate(new_points_list):
                if len(points) > 0:
                    self._points_padded[i, : points.shape[0], :] = points
    ...
    def get_bounding_boxes(self):
        ...
        for points in self.points_list():
            cur_mins = points.min(dim=0)  # (3,)
            cur_maxes = points.max(dim=0)  # (3,)
            all_mins.append(cur_mins)
            all_maxes.append(cur_maxes)
        all_mins = jt.stack(all_mins, dim=0)  # (N, 3)
        all_maxes = jt.stack(all_maxes, dim=0)  # (N, 3)
        bboxes = jt.stack([all_mins, all_maxes], dim=2)
        return bboxes
    ...
    def estimate_normals(
        ...
        if assign_to_self:
            ...
            if self._points_packed is not None:
                ...
                self._normals_packed = jt.cat(self._normals_list, dim=0)
