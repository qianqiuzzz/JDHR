...
#import torch
import jittor as jt
from easyvolcap.structures.bucketize import bucketize
"""
...
def list_to_padded(
    x: Union[List[jt.Var], Tuple[jt.Var]],
    ...
    if not all(jt.is_var(y) for y in x):
        raise ValueError("All items have to be instances of a jt.Var.")
    ...
    x = [
        (y.new_zeros([0] * element_ndim) if (y.ndim == 1 and y.numpy().size == 0) else y)
        for y in x
    ]
...
def list_to_packed(x: List[jt.Var]):
    r"""
    ...
    num_items = jt.array(sizes, dtype=jt.int64)
    ...
    item_packed_to_list_idx = (
        bucketize(item_packed_to_list_idx, item_packed_first_idx, right=True) - 1
    )
...
def packed_to_list(x: jt.Var, split_size: Union[list, int]):
    r"""
...
def padded_to_packed(
    ...
    padded_to_packed_idx = jt.cat(
        [
            jt.arange(v, dtype=jt.int64) + i * M
